 ...
 */
@ScubeService
@Service("NRBcpDebtBussInfoServiceImpl")
public class NRBcpDebtBussInfoServiceImpl implements INRBcpDebtBussInfoService {
	...
	@Override
	@Transactional
	public void addRBcpDebtBussInfo(RBcpDebtBussInfo rBcpDebtBussInfo) throws ScubeBizException {
		if (rBcpDebtBussInfo == null) {
			throw new ScubeBizException("应收类流水信息新增失败！");
		}
	...
	@Transactional
	@Override
	public RBcpDebtBussInfo debtBussInfoAdd(DebtBussInfoVO debtVO) throws ScubeBizException {
		...
		debtBussInfo.setDebtNum(Integer.valueOf(debtVO.getTotalNum().toString()));
		debtBussInfo.setTlrcd(ContextHolder.getUserInfo().getTlrNo());
		debtBussInfo.setBrcode(ContextHolder.getOrgInfo().getBrNo());
