...
public class BusinAnalysisTask {
	...
	public TaskExecResult handle(TaskExecContext context) {
		...
		try {
			...
			String[] params = { "C20170329000224"}; // 传递参数
			this.runJob(uitls.getConn(), params, "经营分析", "/经营分析");
		} catch (Exception e) {
	...
	public static void main(String[] args) throws Exception {
		...
		String[] params = {"C20170329000224"}; // 传递参数
		//执行job
		new BusinAnalysisTask().runJob(uitls.getConn(), params, "经营分析", "/经营分析");
		//执行transfer
		//new FinAnalysisTask().runTransfer(uitls.getConn(), params, "盈利能力", "/经营分析");
	}
}
