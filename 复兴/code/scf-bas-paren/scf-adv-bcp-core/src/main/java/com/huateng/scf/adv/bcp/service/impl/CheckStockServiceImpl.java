...
import com.huateng.scf.adv.bcp.service.ICheckStockService;
...
//		System.out.println("***************arr*****************"+arrBr.toString()); 
		...
		if(list!=null&&list.size()>0){
			...
			while(iter.hasNext()){
//				CheckStockVO  checkVO= new CheckStockVO();
...
//				BeanUtils.copyProperties(VO, checkVO);
	...
	@Override
	public Page threeCheckStockDebetByPage(int pageNo, int pageSize, Map<String, String> param)
			throws ScubeBizException {
	...
	@Override
	public int threeCheckStockNum(Map<String, String> param) throws ScubeBizException {
		return aBcpMoreDvyChkStockDAO.getThreeCheckStockDebetNum(param);
	}
	
	@Override
	public List<com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO> threeCheckStockDebt(Map<String, String> param) throws ScubeBizException {
	...
	@Override
	@Transactional
	public void checkStockApply(ThreeProtocolVO threeProtocolVO,
			List<com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO> debetList) {
    // 保存对账信息
	  String appno = this.saveCheckStockInfo(threeProtocolVO, debetList);
	//保存通知书对账基本信息 B_NTE_NOTICE_BASE
	  BNteNoticeBase noticeBaseInfo = this.saveThreeCheckStockBaseNotice(appno,threeProtocolVO, debetList);
	//保存通知书对账借据信息
	  this.saveThreeCheckStockLnciNotice(noticeBaseInfo, debetList);
		
	}
	 
	/**保存对账借据信息**/
	private void saveThreeCheckStockLnciNotice(BNteNoticeBase noticeBaseInfo,
			List<com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO> debetList) {
		for(com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO lncivo:debetList){
			...
			if(sumQuantity!=null&&sumQuantity.size()>0){
				String num = sumQuantity.get("sum");
				if(null!=num){
	...
	/**保存通知书对账基本信息*/
	private BNteNoticeBase saveThreeCheckStockBaseNotice(String appno, ThreeProtocolVO vo,
		List<com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO> debetList) {
		// 获取当前用户 
		UserInfo user = ContextHolder.getUserInfo(); 
		...
		noticeBaseInfo.setNoticeType(NoticeConstant.NOTICE_TYPE_88);
	 ...
	 */
	public String saveCheckStockInfo(ThreeProtocolVO vo ,List<com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO> debetList) {
		 // 获取当前用户 
		UserInfo user = ContextHolder.getUserInfo(); 
		String tlrNo=user.getTlrNo();
		String brCode=user.getBrNo();
	...
	@Override
	@Transactional
	public void checkStockVerify(com.huateng.scf.adv.bcp.model.CheckStockVO baseInfo) {
		 //更新对账状态
		updateCheckStockInfo(baseInfo);
		updaterNoticeStatus(baseInfo.getAppno(),NoticeConstant.NOTICE_BASE_STATUS_CONFIRM_AGREE);
	}

    /***更新对账状态***/
    public void updateCheckStockInfo(com.huateng.scf.adv.bcp.model.CheckStockVO baseInfo){
    	String date = ScfDateUtil.formatDate(new Date());
		ABcpMoreDvyChkStock chkStock = new ABcpMoreDvyChkStock();
	...
	@Override
	public Map<String, Object> checkStockDebtSum(Map<String, String> param) throws ScubeBizException {
		List<com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO> list =  threeCheckStockDebt(param);
		...
		for(com.huateng.scf.adv.bcp.model.ThreeCheckStockDebetVO vo : list){
			lnciAmtSum = lnciAmtSum.add(vo.getLnciAmt());
			amountOutSum = amountOutSum.add(vo.getAmountOut());
			amountInSum = amountInSum.add(vo.getAmountIn());
		}
