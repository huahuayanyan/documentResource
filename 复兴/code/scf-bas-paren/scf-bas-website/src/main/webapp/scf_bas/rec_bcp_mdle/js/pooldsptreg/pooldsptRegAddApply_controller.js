...
var pooldsptRegAddApplyController = function($scope,$common,dataDeliver,$state,$stateParams,$compile,$eventCommunicationChannel) {
                ...
                if (res.errorCode =="SUC"){
                    ...
                    promise2.then(function(res1){
                        if (res1.errorCode =="SUC"){
                            ...
                            var promise = $common.get_asyncData("rBcpDebtBaseInfoService/findRBcpDebtBaseInfoForPDRemove",dataQ);
                            promise.then(function(res){
...
// add,update,del,progress,save,submit,back
    ...
    $scope.del_onClick = function(){
        if($scope.bcpSelected.select.length != 1){
            ...
            var primiseA = $common.get_asyncData('rBcpPoolDisputeRemoveService/deleteRBcpDebt',{rBcpDebtBussDtl:selected});
            primiseA.then(function(res){
                if (res.errorCode =="SUC"){
    ...
    $scope.save_onClick= function(){
        if($.doValidate("cntDebtInfoForm")){//实际上这里还需要再去后台检验下客户名称和客户号是否一致 todo!!!!
            if($scope.bcpDatasource.ds==[]){
                ...
                if(flag){
                    for(var k in $scope.bcpDatasource.ds){
                        if(!$scope.bcpDatasource.ds[k].money || $scope.bcpDatasource.ds[k].money==""){
                            flag = false;
