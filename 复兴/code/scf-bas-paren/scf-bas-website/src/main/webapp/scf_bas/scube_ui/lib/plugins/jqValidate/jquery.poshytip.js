...
(function($) {
	...
	//$(window).scroll(handleWindowResize);
	...
	$.Poshytip.prototype = {
		...
		refresh: function(async) {
			...
			// reset position to avoid text wrapping, etc.
             //update by wubin 2017-03-30
            var isnest = false;
            var parentDialog;
            this.$elm.parents().each(function(index,parent){
                if($(parent).hasClass('ngdialogwrap')){
                    isnest = true;
                    parentDialog = parent;
                    this.$tip.css({left: 0, top: 0}).appendTo($(parent));
                }
            }.bind(this));
            if(!isnest){
                this.$tip.css({left: 0, top: 0}).appendTo(document.body);
            }
             //update by wubin 2017-03-30 结束
        ...
        getscroll: function(src) { //add by wubin 2017-03-30
        ...
        getoffset: function(src,target) { //add by wubin 2017-03-30
		...
		calcPos: function() {
            ...
            if(arguments[0]){
                $win = $(arguments[0]);
                var _elmOffset = this.getoffset(this.$elm[0],arguments[0]);
                elm.l = _elmOffset[1];
                elm.t = _elmOffset[0];
            };
            var win = {
