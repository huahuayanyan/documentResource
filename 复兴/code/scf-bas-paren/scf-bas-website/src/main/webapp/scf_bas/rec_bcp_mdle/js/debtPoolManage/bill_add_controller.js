            ...
            if($.doValidate("billForm")){
                ...
                if(errorflag){
                    ...
                    if(flag == "update"){
                        ...
                        /* var contAmount=parseFloat(row.contAmount);//合同额度
                         var totalDebtAmount=0;//总金额
                         var datasource=parentScope.datasource.ds
                         if(datasource[0]){//判断前一个页面的有没有数据
                         for(var k in datasource){
                         totalDebtAmount=totalDebtAmount+parseFloat(datasource[k].billsAmountView)
                         }
                         }
                         totalDebtAmount=totalDebtAmount+parseFloat(dialogScope.billForm.billsAmountView);
                         //判断下有效金额不能大于合同金额
                         if(totalDebtAmount >contAmount ){
                         $common.get_tipDialog("凭证单据票面金额总和不能大于合同金额！",'showError');
                         }else{
                         parentScope.datasource.ds.push(dialogScope.billForm);
                         dialogScope.closeThisDialog();
                         }*/
