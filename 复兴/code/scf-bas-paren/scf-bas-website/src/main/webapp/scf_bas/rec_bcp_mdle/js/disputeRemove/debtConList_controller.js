 ...
 */
var DebtConListController = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
	$scope.conditionForm={
   ...
   $scope.queryTable = function(){
    ...
    //申请转让,因为用的是别人的代码，这里的字段不对应并且还少一些，还的到数据库里查询下。
            ...
            promise.then(function(res){
                ...
                if (res.errorCode =="SUC"){
                    if(res.contextData.value != "0"){
                        ...
                        $common.get_tipDialog("该合同不存在可解除纠纷的应收账款信息！",'showError');
                    }
                }else{
                    $common.get_tipDialog(res.errorMsg,'showError');
                }
