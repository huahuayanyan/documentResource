 ...
 */
var DebtTransferController = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
    var dataX=dataDeliver.getter();
    var debtInfo=dataX.debtInfo;
    $scope.otherformX={};
    $scope.otherformX.ext2='应收账款转让申请';
   ...
   $scope.queryTable = function(){
        ...
        if ($scope.debtExtselected.select.length != 1) {
            ...
            $common.get_tipDialog('确定删除吗！', 'showTip').then(function () {
                ...
                for (var i = 0; i < dsArray.length; i++) {
                    if (dsArray[i].custcd == selected.custcd) {
                        del_buyerCustcd.push(selected.custcd);
    ...
    //删除票据信息(要考虑到当删除票据之后，按比例收费的费用小于减免金额，这时费用计算错误)
    ...
    $scope.save_onClick=function(){
        if($.doValidate("otherform")) {
            ...
            var data33 = {
                modelId: 'FactoringAanwinstApply',
                ProcessName: '应收账款转让申请',
                NodeId: 'Write',
