...
import com.huateng.scf.adv.nte.service.IGuaranBuyNoticeService;
...
@ScubeService
@Service("GuaranBuyNoticeServiceImpl")
public class GuaranBuyNoticeServiceImpl implements IGuaranBuyNoticeService {
	private final Logger log = LoggerFactory.getLogger(getClass());
	@Resource(name = IGuaranBuyNoticeService.BEAN_ID)
	IGuaranBuyNoticeService guaranBuyNoticeService; 
	...
	public Page queryGuaranBuyNteByPage(int pageNo, int pageSize, Map<String, String> param) throws ScubeBizException {
		log.info("发货对账通知书查询");
		log.info("差额退款通知书查询");
		...
		if (StringUtils.isNotEmpty(param.get("protocolNo"))) {
			...
			criteria.andProtocolNoLike("%"+param.get("protocolNo")+"%");//0003300:“担保提货协议号”模糊查询
		}
		...
		if (StringUtils.isNotEmpty(param.get("status"))) {
			criteria.andStatusEqualTo(param.get("status"));
		}
...
//			criteria.andBrcodeEqualTo(param.get("brcode"));
		...
		example.setOrderByClause("APPNO desc, SIGN_DATE desc");
