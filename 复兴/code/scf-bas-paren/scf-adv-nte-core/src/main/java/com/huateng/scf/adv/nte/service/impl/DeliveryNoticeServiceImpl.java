...
import com.huateng.scf.adv.nte.service.IDeliveryNoticeService;
...
@ScubeService
@Service("DeliveryNoticeServiceImpl")
public class DeliveryNoticeServiceImpl implements IDeliveryNoticeService {
	private final Logger log = LoggerFactory.getLogger(getClass());
	@Resource(name = IDeliveryNoticeService.BEAN_ID)
	IDeliveryNoticeService deliveryNoticeService; 
	...
	@Override
	public Page queryDeliveryNteByPage(int pageNo, int pageSize, Map<String, String> param) throws ScubeBizException {
		log.info("提货通知书查询");
		...
		if (StringUtils.isNotEmpty(param.get("protocolNo"))) {
			criteria.andProtocolNoLike("%"+param.get("protocolNo")+"%");
		}
		if (StringUtils.isNotEmpty(param.get("debetId"))) {
			criteria.andDebetIdLike("%"+param.get("debetId")+"%");
		}
...
//			criteria.andBrcodeEqualTo(param.get("brcode"));
		...
		example.setOrderByClause("NUM desc, SIGN_DATE desc");//0003290
