...
//差额退款通知书列表
var dvyChkNoticeQryController = function ($scope, $common, $state, dataDeliver) {

    $scope.conditionForm = {productId:"1210010",
        noticeType:"88"
    };//初始化查询条件对象
    $scope.productSource={
    		ds:[{code:"1210010",codeName:"担保提货"}]
    };
 /*   productSource();*/
...
//查看操作！
    $scope.column = function(row){
        $common.get_ngDialog('dvyChkNotice/dvyChk_notice_qry_view.html', $scope
            ,["$scope",function(dialogScope){
                dvyChkNoticeQryViewController(dialogScope,$scope,$common,row);
            }],"发货对账通知书基本信息",{showClose:true});
    };
    //重置按钮
    $scope.rebort_onClick = function () {
        $scope.$apply(function(){
            $scope.conditionForm = {productId:"1210010",noticeType:"88"};
