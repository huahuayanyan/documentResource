 ...
 */
var stockListImportApplyController = function($scope,$common,dataDeliver,$state,$eventCommunicationChannel) {
    $scope.conditionForm={
        conStat:"1",
        vidPrdId:"1220020"
    };//初始化查询条件对象
    ...
    $scope.startApply_onClick = function()
    {
        if($scope.selected.select.length != 1){
            ...
            $common.get_asyncData("stockListImportApplyService/DeliveryApplyQryGetter", data).then(function (res) {
                var baseinfo;
                if (res.errorCode == "SUC") {
