/* 浏览器支持情况说明
1）IE;7、8不支持，9支持
2）chrome、firefox、opera支持
 */
// document.write("<script language='JavaScript' src='/GooFlow/GooFlow/jquery-1.10.1.min.js'>");
var GooFunc = {
...
/**
* 定义一个区域图类
* @params: bgDiv 最外层容器 Jquery 对象
* @params: property 用户传入的自定义参数
*/
function GooFlow(bgDiv,property){
    ...
    // 必要参数校验
    var eleName = property.id || bgDiv.attr("id");
    if (!eleName) {
    ...
    this.$modelId = null;       // 流程图的ID
    this.$title = null;         // 流程图的名称
    this.$nodeRemark = {};      // 每一种结点或按钮的说明文字,JSON格式,key为类名,value为用户自定义文字说明
    this.$nowType = "cursor";   // 当前要绘制的对象类型
    this.$mainArea = null;      // 左侧主区域
    this.$propertyArea = null;  // 属性区域
    this.$modelDes = property.modelDes;     // 流程定义
    this.$nodeDes = property.nodeDes;       // 节点定义
    this.$taskDes = property.taskDes;       // 任务定义
    this.showModelDes = property.showModelDes;
    this.showNodeDes = property.showNodeDes;
    this.showTaskDes = property.showTaskDes;
    this.saveData = property.saveData;
    this.$detailHeight = property.detailHeight;
    this.$nodeDetail = property.nodeDetail;
    this.$lineData = {};
    this.$lineCount = 0;
    this.$nodeData = {};
    this.$nodeCount = 0;
    this.$lineDom = {};
    this.$nodeDom = {};
    this.$max = property.initNum;        // 计算默认ID值的起始SEQUENCE
    ...
    this.$curItemId = null;
    this.$sysId = null;
    this.$oldModelId = null;

    var width;
    if (property.width == 'auto'){
        width = this.$bgDiv.parent().width();
    } else {
    ...
    if (property.editable) {
        width -= 350;
        HtmlStr = "<div id='main' class='GooFlow'></div>";
        this.$mainArea = $(HtmlStr);
        this.$mainArea.css({width:width+"px",height:height+"px",float:"left",clear:"left"});
        this.$bgDiv.append(this.$mainArea);
        HtmlStr = "<div id='property' class='GooFlow GooFlow_property' style='height:"+height+"px'></div>";
        this.$propertyArea = $(HtmlStr);
        this.$propertyArea.append(this.$modelDes);
        this.$propertyArea.append(this.$nodeDes);
        this.$propertyArea.append(this.$taskDes);
        this.$bgDiv.append(this.$propertyArea);
        if (property.haveHead) {
            HtmlStr = "<div class='GooFlow_head'><div class='GooFlow_head_title'><div class='GooFlow_head_title_content'>"+property.initLabelText+"</div><div class='GooFlow_head_title_point'></div></div>";
            for (var x=0; x<property.headBtns.length; ++x) {
                HtmlStr += "<a href='javascript:void(0)' title='"+(property.headBtnRemarks[x]||property.headBtns[x])+"'>";
                HtmlStr += "<i class='fa ico_"+property.headBtns[x]+"'></i></a>";
            }
            HtmlStr += "</div>";
            this.$head = $(HtmlStr);
            this.$mainArea.append(this.$head);
            height -= 39;
            //以下是当工具栏按钮被点击时触发的事件自定义(虚函数),格式为function(),因为可直接用THIS操作对象本身,不用传参；用户可自行重定义:
            this.onBtnNewClick     = property.onBtnNewClick;        // 新建流程图按钮定义
            this.onBtnOpenClick    = property.onBtnOpenClick;       // 打开流程图按钮定义
            this.onBtnSaveClick    = property.onBtnSaveClick;       // 保存流程图按钮定义
            this.onFreshClick      = property.onFreshClick;       // 重载流程图按钮定义
            this.onExpandClick     = property.onExpandClick;     // 扩大画布按钮定义
            this.onBtnImportClick = property.onBtnImportClick;  // 导入流程图按钮定义
            this.onBtnExportClick = property.onBtnExportClick;  // 导出流程图按钮定义
            if (property.headBtns) {
                this.$head.on("click", {inthis: this}, function (e) {
                    ...
                    //定义顶部操作栏按钮的事件
                    var icoName = $(tar).attr("class").match(/ico_\w+/g)[0];
                    switch (icoName) {
                        ...
                        case "ico_expand":  if (This.onExpandClick != null)     This.onExpandClick();        break;
        ...
        if (property.haveTool) {
            HtmlStr = "<div class='GooFlow_tool' style='height: " + height + "px'></div>";
            this.$mainArea.append(HtmlStr);
            this.$tool = this.$mainArea.find(".GooFlow_tool");
            HtmlStr = "<a href='javascript:void(0)' id='" + this.$id + "_btn_cursor'><i class='fa ico_cursor'/></a>"
                + "<div class='remark'>" + (property.toolBtnRemarks['cursor'] ? property.toolBtnRemarks['cursor'] : "") + "</div>"
                + "<a href='javascript:void(0)' id='" + this.$id + "_btn_direct'><i class='fa ico_direct'/></a>"
                + "<label>" + (property.toolBtnRemarks['direct'] ? property.toolBtnRemarks['direct'] : "") + "</label>";
            this.$tool.append(HtmlStr);
            if (property.toolBtns && property.toolBtns.length > 0) {
                HtmlStr = "<span/>";
                for (var i=0; i<property.toolBtns.length; i++) {
                    HtmlStr += "<a href='javascript:void(0)' id='" + this.$id + "_btn_" + property.toolBtns[i] + "'><i class='fa ico_" + property.toolBtns[i] + "'/></a>"
                        + "<label>" + (property.toolBtnRemarks[property.toolBtns[i]] ? property.toolBtnRemarks[property.toolBtns[i]] : "") + "</label>";
                }
                this.$tool.append(HtmlStr);
            }
            width -= 84;
        ...
        } else {//高级浏览模式(editable=true,havetool=false),可查看节点的属性或 任务(在任务节点情况下可见)
            var areas = this.$propertyArea.children();
            for (var i = 0; i < areas.length; i++) {
                var list = $(areas.get(i)).children('input');
                if (list.length != 0) {
                    for (var n = 0; n < list.length; n++) {
                        var item = list.get(n);
                        $(item).get(0).setAttribute("disabled", "true");
                    }
                }
                list = $(areas.get(i)).children('select');
                if (list.length != 0) {
                    for (var n = 0; n < list.length; n++) {
                        var item = list.get(n);
                        $(item).get(0).setAttribute("disabled", "true");
                ...
                list = $(areas.get(i)).children('#taskRemark');
                if (list.length != 0) {
                    for (var n = 0; n < list.length; n++) {
                        var item = list.get(n);
                        $(item).get(0).setAttribute("disabled", "disabled");
                        $(item).get(0).setAttribute("readonly", "true");
                    }
                }
                if (i == 2) {
                    list = $(areas.get(i)).children('div');
                    for (var n = 0; n < list.length; n++) {
                        var select = $(list.get(n)).children('select');
                        if (select.length != 0) {
                            select.get(0).setAttribute('disabled', 'true');
                        }
                    }
                }
            }
        ...
        HtmlStr = "<div id='main' class='GooFlow'></div>";
        this.$mainArea = $(HtmlStr);
        this.$mainArea.css({width:width+"px",height:height+"px"});
        this.$bgDiv.append(this.$mainArea);
    }
    var workWidth = property.workWidth;
    var workHeight = property.workHeight;
    if (!workWidth)     workWidth = width;
    if (!workHeight)    workHeight = height;
    this.$mainArea.append("<div class='GooFlow_work' style='width:"+(width)+"px;height:"+(height)+"px'></div>");
    ...
    this.$mainArea.children(".GooFlow_work").append(this.$workArea);
    ...
    if (this.$editable) {
        ...
        this.$mainArea.append(this.$ghost);
            ...
            document.onmousemove = function(e) {
                ...
                if (This.$lineMove.data("type") == "lr") {
                    ...
                    if (X < 0)  X = 0;
                        ...
                        if (X > This.$workArea.width())
                            X = This.$workArea.width();
                    This.$lineMove.css({left:X+"px"});
                } else 
                    if (This.$lineMove.data("type") == "tb") {
                        Y = Y - vY;
                        if (Y < 0)  Y = 0;
                        else 
                            if (Y > This.$workArea.height())
        ...
        this.$lineOper.on("click", {inthis:this}, function(e) {
            ...
            switch ($(e.target).attr("class")) {
                case "b_x":     This.delLine(id);           this.style.display = "none";    break;
                case "b_l1":    This.setLineType(id,"lr");  break;
                case "b_l2":    This.setLineType(id,"tb");  break;
                case "b_l3":    This.setLineType(id,"sl");  break;
            }
        });
        //下面绑定当结点/线/分组块的一些操作事件,这些事件可直接通过this访问对象本身
        //当操作某个单元（结点/线/分组块）被添加时, 触发的方法, 返回FALSE可阻止添加事件的发生
        //格式function(id, type,json)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值,json即addNode,addLine方法的第二个传参json.
        this.onItemAdd = property.onItemAdd;
        //当操作某个单元（结点/线/分组块）被删除时, 触发的方法, 返回FALSE可阻止删除事件的发生
        //格式function(id, type)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值
        this.onItemDel = property.onItemDel;
        //当操作某个单元（结点/分组块）被移动时, 触发的方法, 返回FALSE可阻止移动事件的发生
        //格式function(id, type,left,top)：id是单元的唯一标识ID,type是单元的种类,有"node"一种取值, 线line不支持移动,left是新的左边距坐标, top是新的顶边距坐标
        this.onItemMove = property.onItemMove;
        //当操作某个单元（结点/线/分组块）被重命名时, 触发的方法, 返回FALSE可阻止重命名事件的发生
        //格式function(id,name,type)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值,name是新的名称
        ...
        //
        this.onItemDClick = property.onItemDClick;
        ...
        //格式function(id, type,width,height)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值;width是新的宽度,height是新的高度
        ...
        //ScubeUI提示框
        this.showTipDialog = property.showTipDialog;
        ...
        //绑定双击编辑事件
        this.$workArea.delegate(".GooFlow_item","dblclick",{inthis:this},function(e){
            if (!e)  e = window.event;
            if (e.data.inthis.onItemDClick != null)
                e.data.inthis.onItemDClick(this.id);
        });
        if (property.haveDetail == true) {
            this.$mainArea.append("<div class='GooFlow_detail' style='height:" + this.$detailHeight + "px'></div>");
            this.$mainArea.children(".GooFlow_detail").append(this.$nodeDetail);
        }
    }
}
GooFlow.prototype = {
    ...
    initDraw: function(id, width, height, workWidth, workHeight) {
        ...
        if (GooFlow.prototype.useSVG != "") {
            ...
            this.$workArea.prepend("<div class='GooFlow_work_vml' style='position:relative;width:"+workWidth+"px;height:"+workHeight+"px'></div>");
            this.$workArea.children("div")[0].insertBefore(this.$draw,null);
        }
        this.$draw.id = id;
        this.$draw.style.width = workWidth + "px";
        this.$draw.style.height = +workHeight + "px";
        //绑定连线的点击选中以及双击编辑事件
        var tmpClk = null;
        if (GooFlow.prototype.useSVG != "")  
            tmpClk="g";
        ...
        if (this.$editable) {
            $(this.$draw).delegate(tmpClk, "dblclick", {inthis: this}, function (e) {
                ...
                var t = GooFunc.getElCoordinate(This.$workArea[0]), c = GooFunc.getElCoordinate(This.$MainContener);
    ...
    switchToolBtn: function(type) {
        ...
        var ico = $(this.$tool.children("#"+this.$id+"_btn_"+this.$nowType)[0]).children();
        if (ico[0].className.split("_down").length>1) {
            ico.attr("class","ico_"+this.$nowType);
        }
        this.$nowType = type;
        ico = $(this.$tool.children("#"+this.$id+"_btn_"+this.$nowType)[0]).children();
        ico.attr("class","ico_"+this.$nowType+"_down");
    },
    ...
    addNode: function(id, json) {
        ...
        json.width = json.width ? json.width : 82;
        json.height = json.height ? json.height : 82;
        if (json.top<0) 
            json.top = 0;
        if (json.left<0)   
            json.left = 0;
        ...
        if (navigator.userAgent.indexOf("8.0") != -1) 
            hack = 2;
        this.$nodeDom[id] = $("<div class='GooFlow_item' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'>"
            + "<div class='icoDiv "+mark+"'><i class='fa ico_"+json.type+"'></i><div class='rs_close'></div></div>"
            + "<label style='width:60px;display:block;overflow:hidden;'>"+json.name+"</label></div>");
    ...
    initWorkForNode: function() {
        ...
        this.$workArea.delegate(".GooFlow_item", "click", {inthis: this}, function(e) {
            e.data.inthis.focusItem(this.id, true);
            $(this).children("div").removeClass("item_mark");
        });
        //绑定双击编辑事件
        this.$workArea.delegate(".GooFlow_item","dblclick",{inthis:this},function(e){
            if (!e)  e = window.event;
        ...
        // this.$workArea.delegate(".node_name","dblclick",{inthis:this},function(e){});
        if (this.$tool == null)
            return;
        //绑定用鼠标移动事件
        this.$workArea.delegate(".icoDiv", "mousedown", {inthis: this}, function(e) {
            if (!e) e = window.event;
            if ($(e.target).attr("class")=="rs_close")
                return;
            var This = e.data.inthis;
            ...
            if (navigator.userAgent.indexOf("8.0") != -1)
                hack = -1;
            var ev = GooFunc.mousePosition(e), t = GooFunc.getElCoordinate(This.$workArea[0]), c = GooFunc.getElCoordinate(This.$MainContener);
            This.$ghost.css({
                display: "block",
                top: This.$nodeData[id].top+t.top-c.top-This.$workArea[0].parentNode.scrollTop+hack+"px",
                left: This.$nodeData[id].left+t.left-c.left-This.$workArea[0].parentNode.scrollLeft+hack+"px",
                cursor:"move"
            });
            Dom.children("ico").clone().prependTo(This.$ghost);
            var X, Y;
            X = ev.x - t.left + This.$workArea[0].parentNode.scrollLeft;
            Y = ev.y - t.top + This.$workArea[0].parentNode.scrollTop;
            ...
            document.onmousemove = function(e) {
                ...
                if (X < t.left-c.left - This.$workArea[0].parentNode.scrollLeft)
                    ...
                    if (X+This.$workArea[0].parentNode.scrollLeft+This.$nodeData[id].width > t.left-c.left+This.$workArea.width())
                ...
                if (Y < t.top-c.top-This.$workArea[0].parentNode.scrollTop)
                    ...
                    if (Y+This.$workArea[0].parentNode.scrollTop+This.$nodeData[id].height > t.top-c.top+This.$workArea.height())
            ...
            document.onmouseup = function(e) {
                if (isMove)
                    This.moveNode(id, X+This.$workArea[0].parentNode.scrollLeft-t.left+c.left, Y+This.$workArea[0].parentNode.scrollTop-t.top+c.top);
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseenter", {inthis: this}, function(e) {
            if (e.data.inthis.$nowType != "direct")
                return;
            $(this).children("div.icoDiv").addClass("item_mark");
        });
        this.$workArea.delegate(".GooFlow_item", "mouseleave", {inthis: this}, function(e) {
            if (e.data.inthis.$nowType != "direct")
                return;
            $(this).children("div.icoDiv").removeClass("item_mark");
        });
        //绑定连线时确定初始点
        ...
        this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
            ...
            if(lineStart)   This.addLine("line_"+This.$max,{from:lineStart.id,to:this.id,name:""});
            This.$max++;
        });
        //绑定结点的删除功能
    ...
    blurItem:function(){
        if(this.$focus!=""){
            ...
            if(jq.prop("tagName")=="DIV"){
                var id=jq.attr('id');
                if (this.$editable) {
                    var nodeData = this.saveData().nodeDes;
                    if (id != nodeData.nodeName)
                        this.setName(id, nodeData.nodeName, "node");
                    this.$nodeData[id].nodeId = nodeData.nodeId;
                    this.$nodeData[id].name = nodeData.nodeName;
                    this.$nodeData[id].nodeSeq = nodeData.nodeSeq;
                    this.$nodeData[id].expDes = nodeData.expDes;
                    this.$nodeData[id].expParam = nodeData.expParam;
                    this.$nodeData[id].callerClasspath = nodeData.callerClasspath;
                    if(this.$nodeData[id].type == "task") {
                        $("#taskDes").css("display","none");
                        var taskData = this.saveData().taskDes;
                        if (taskData != null && taskData != undefined) {
                            if (taskData.taskId != null && taskData.taskId != undefined && taskData.taskId != '')
                                this.$nodeData[id].taskdes.taskId = taskData.taskId;
                            if (taskData.taskName != null && taskData.taskName != undefined && taskData.taskName != '')
                                this.$nodeData[id].taskdes.taskName = taskData.taskName;
                            if (taskData.assignStrategy != null && taskData.assignStrategy != undefined && taskData.assignStrategy != '')
                                this.$nodeData[id].taskdes.assignStrategy = taskData.assignStrategy;
                            if (taskData.unitLevel != null && taskData.unitLevel != undefined && taskData.unitLevel != '')
                                this.$nodeData[id].taskdes.unitLevel = taskData.unitLevel;
                            if (taskData.unitId != null && taskData.unitId != undefined && taskData.unitId != '')
                                this.$nodeData[id].taskdes.unitId = taskData.unitId;
                            if (taskData.roleId != null && taskData.roleId != undefined && taskData.roleId != '')
                                this.$nodeData[id].taskdes.roleId = taskData.roleId;
                            if (taskData.userIds != null && taskData.userIds != undefined && taskData.userIds != '')
                                this.$nodeData[id].taskdes.userIds = taskData.userIds;
                            if (taskData.taskDescription != null && taskData.taskDescription != undefined && taskData.taskDescription != '')
                                this.$nodeData[id].taskdes.taskDescription = taskData.taskDescription;
                        }
                    }
                    $("#nodeDes").css("display","none");
                    $("#modelDes").css("display","block");
                }
                if(this.onItemBlur!=null&&!this.onItemBlur(id,"node"))  return false;
                var ico=$("#"+id+" div:first").removeClass("item_focus");
                ico.children("div:eq(0)").css("display","none");
            }
            else{
    ...
    focusItem:function(id,bool){
        ...
        if(jq.prop("tagName")=="DIV"){
            ...
            $("#"+id+" div:first").addClass("item_focus");
            if(this.$editable)
                $("#"+id+" div:first").children("div:eq(0)").css("display","block");
            this.$workArea.append(jq);
            if (this.$editable) {
                // 显示节点属性值 start
                $("#modelDes").css("display", "none");
                $("#nodeDes").css("display", "block");
                var nodeDes = {
                    nodeId: this.$nodeData[id].nodeId,
                    nodeName: this.$nodeData[id].name,
                    nodeType: this.$nodeData[id].type,
                    nodeSeq: this.$nodeData[id].nodeSeq,
                    expDes: this.$nodeData[id].expDes,
                    expParam: this.$nodeData[id].expParam,
                    callerClasspath: this.$nodeData[id].callerClasspath
                };
                this.showNodeDes(nodeDes);
                if (this.$nodeData[id].type == "task") {
                    $("#taskDes").css("display", "block");
                    var thisTaskDes = {
                        taskId: '',
                        taskName: '',
                        assignStrategy: '',
                    ...
                    var taskDesData = this.$nodeData[id].taskdes;
                    if (taskDesData != null && taskDesData != undefined) {
                        if (taskDesData.taskId != null && taskDesData.taskId != undefined)
                            thisTaskDes.taskId = taskDesData.taskId;
                        if (taskDesData.taskName != null && taskDesData.taskName != undefined)
                            thisTaskDes.taskName = taskDesData.taskName;
                        if (taskDesData.assignStrategy != null && taskDesData.assignStrategy != undefined)
                            thisTaskDes.assignStrategy = taskDesData.assignStrategy;
                        if (taskDesData.unitLevel != null && taskDesData.unitLevel != undefined)
                            thisTaskDes.unitLevel = taskDesData.unitLevel;
                        if (taskDesData.unitId != null && taskDesData.unitId != undefined)
                            thisTaskDes.unitId = taskDesData.unitId;
                        if (taskDesData.roleId != null && taskDesData.roleId != undefined)
                            thisTaskDes.roleId = taskDesData.roleId;
                        if (taskDesData.userIds != null && taskDesData.userIds != undefined)
                            thisTaskDes.userIds = taskDesData.userIds;
                        if (taskDesData.taskDescription != null && taskDesData.taskDescription != undefined)
                            thisTaskDes.taskDescription = taskDesData.taskDescription;
                    }
                    switch (thisTaskDes.assignStrategy) {
                        case "0":
                            thisTaskDes.userIds = '';
                            break;
                        case "1":
                            thisTaskDes.unitLevel = '';
                            thisTaskDes.userIds = '';
                            break;
                        case "2":
                            thisTaskDes.unitLevel = '';
            ...
            //var t=GooFunc.getElCoordinate(this.$workArea[0]);
    ...
    setName:function(id,name,type){
        ...
        if (type == "node") {//如果是结点
            ...
            this.$nodeDom[id].find("label")[0].textContent = name;
            var hack = 0;
    ...
    //修改节点的类型
    setNodeType:function(id,type,newtype){
        if(!this.$nodeData[id]) return;
        if(this.$nodeData[id].type!=type) return;
        if(this.$nodeData[id].type==newtype) return;
        if(this.$nodeData[id].type=="start"||this.$nodeData[id].type=="end") return;
        if(newtype=="start"||newtype=="end") return;
        this.$nodeData[id].type=newtype;
        this.$nodeDom[id].find('.ico').children('i').removeClass('ico_'+type).addClass('ico_'+newtype);
        if(this.$undoStack){
            var paras=[id,newtype,type];
            this.pushOper("setNodeType",paras);
        }
    },
    //删除结点
    delNode:function(id){
    ...
    //设置流程图的名称
    setTitle:function(sysId, modelId, modelName){
        this.$sysId = sysId;
        this.$modelId = modelId;
        this.$title = modelName;
        if (this.$head) {
            if (this.$title != "" && this.$title != null)
                this.$head.children(".GooFlow_head_title").children(".GooFlow_head_title_content")[0].innerText = this.$title;
            else
                this.$head.children(".GooFlow_head_title").children(".GooFlow_head_title_content")[0].innerText = "New Flow";
        }
        if (this.$editable) {
            var modelDes = {sysId: this.$sysId,modelId: modelId, modelName: modelName};
            this.showModelDes(modelDes);
        }
    },
    //载入一组数据
    loadData:function(data){
        this.$sysId = data.sysId;
        this.$oldModelId = data.modelId;
        this.$modelId = data.modelId;
        this.$title = data.modelName;
        this.setTitle(this.$sysId, this.$modelId, this.$title);
        if(data.initNum)    this.$max=data.initNum;
        for(var i in data.nodes){
            this.addNode(i,data.nodes[i]);
            var ThisSeq = i.split('node_')[1] * 1;
            if(ThisSeq && this.$max < ThisSeq)
                this.$max = ThisSeq;
        }
        for(var j in data.lines){
            this.addLine(j,data.lines[j]);
            var ThisSeq = j.split('line_')[1] * 1;
            if(ThisSeq && this.$max < ThisSeq)
                this.$max = ThisSeq;
        }
        this.$max++;
    ...
    //把画好的结束导出到一个变量中(其实也可以直接访问GooFlow对象的$nodeData,$lineData这两个JSON属性)
    exportData:function(){
        this.blurItem();
        var nodeIds=[];
        $.each(this.$nodeData, function (index, value) {
            nodeIds.push(index);
        });
        var lineIds=[];
        $.each(this.$lineData, function (index, value) {
            lineIds.push(index);
        });
        return {
            sysId:this.$sysId,
    ...
    clearData:function(){
        ...
        if (this.$editable && this.$tool != null) {
            this.showModelDes({sysId:'',modelId:'',modelName:''});
            this.showNodeDes({nodeId:'',nodeName:'',nodeSeq:'',nodeType:'',expDes:'',expParam:'',callerClasspath:''});
            this.showTaskDes({taskId:'',taskName:'',assignStrategy:'',unitLevel:'',unitName:'',unitId:'',roleId:'',userIds:'',taskDescription:''});
            $("#modelDes").css("display","block");
            $("#nodeDes").css("display","none");
            $("#taskDes").css("display","none");
        }
    ...
    calcStartEnd:function(n1,n2){
        ...
        X_2 = n2.left + 25;
        Y_2 = n2.top + 25;
        var line = Math.sqrt(Math.pow(X_1-X_2,2)+Math.pow(Y_1-Y_2,2));
        var proportion = 30/line;
        var sp_x = X_1+proportion*(X_2-X_1);
        var sp_y = Y_1+proportion*(Y_2-Y_1);
        var ep_x = X_2+proportion*(X_1-X_2);
        var ep_y = Y_2+proportion*(Y_1-Y_2);
        return {"start":[sp_x,sp_y],"end":[ep_x,ep_y]};
    },
    //计算两个结点间要连折线的话, 连线的所有坐标
    calcPolyPoints:function(n1,n2,type,M){
        //开始/结束两个结点的中心
        var SP={x:n1.left+25,y:n1.top+25};
        var EP={x:n2.left+25,y:n2.top+25};
        var sp=[],m1=[],m2=[],ep=[];
        //如果是允许中段可左右移动的折线,则参数M为可移动中段线的X坐标
        //粗略计算起始点
        var line = Math.sqrt(Math.pow(SP.x-EP.x,2)+Math.pow(SP.y-EP.y,2));
        var proportion = 30/line;
        sp=[SP.x+proportion*(EP.x-SP.x),SP.y+proportion*(EP.y-SP.y)];
        ep=[EP.x+proportion*(SP.x-EP.x),EP.y+proportion*(SP.y-EP.y)];
        if(type=="lr"){
            //粗略计算2个中点
            m1=[M,SP.y];
            m2=[M,EP.y];
    ...
    addLine:function(id,json){
        ...
        n1=this.$nodeData[json.from],n2=this.$nodeData[json.to];//获取开始/结束结点的数据
        if(!n1||!n2)    return;
        var res;
...
// 将 createGooFlow 注册成为 jq 插件
$.fn.extend({
    createGooFlow: function (property) {
        var defaultOption = {
            id: '',
            // 如果工作区并非包含在position:absulote的情况下，保持默认即可
            // 但是如果外围存在这种绝对定位的容器，则此处应该添加该绝对定位容器的dom
            MainContener: $('body')[0],
            width: 'auto',    // 此两个变量定义整个控件的宽高（含工具栏、滚动条）, 
            height: 500,   // 如忽略, 则默认宽度=容器宽度
            initNum: 1,    // 元素起始 编码
            initLabelText: 'New Flow',
            workWidth: null,    // 此两个变量定义画布的宽高, 如忽略, 则这两个变量根据 width 和 height 自动计算。
            workHeight: null,   // 目的是不产生拖动/滚动条, 刚好适配宽高
            haveDetail: false,  // editable为false时生效
            detailHeight: 200,  // editable为false,haveDetail为true时生效
            nodeDetail: null,  // editable为false,haveDetail为true时生效，若需要在此显示Scube内容，可在父div之前定义元素，再将JQ对象赋值给nodeDetail属性
            toolBtns: ["base", "task", "end", "appro", "auto"],
            toolBtnRemarks: {cursor:"选择指针", direct:"连线", base:"基本节点", task:"任务节点", end:"结束节点",
                appro:"审批节点", auto:"自动节点"},
            haveHead: true,      //是否显示头部工具栏
            haveTool: true,      //是否显示左侧工具栏，仅当editable为true时有效。显示时右侧属性可编辑，不显示时右侧属性不可编辑
            headBtns: ["new", "open", "save", "reload", "undo", "redo", "expand", "import", "export"],
            headBtnRemarks: ['新建', '打开', '保存', '重置', '撤销', '重做', '扩大画布', '导入', '导出'],
            ...
            onExpandClick: function(){
                this.reinitSize(this.workHeight,this.workHeight);
            },
            onBtnImportClick: null, // 导入流程图按钮定义
            onBtnExportClick: null, // 导出流程图按钮定义
            //当操作某个单元（结点/线/分组块）被添加时, 触发的方法, 返回FALSE可阻止添加事件的发生
            //格式function(id, type, json)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值,json即addNode,addLine方法的第二个传参json.
            onItemAdd: null,
            //当操作某个单元（结点/线/分组块）被删除时, 触发的方法, 返回FALSE可阻止删除事件的发生
            ...
            onItemDel: null,
            //当操作某个单元（结点/分组块）被移动时, 触发的方法, 返回FALSE可阻止移动事件的发生
            //格式function(id, type, left, top)：id是单元的唯一标识ID,type是单元的种类,有"node"一种取值, 线line不支持移动,left是新的左边距坐标,
            // top是新的顶边距坐标
            onItemMove: null,
            //当操作某个单元（结点/线/分组块）被重命名时, 触发的方法, 返回FALSE可阻止重命名事件的发生
            //格式function(id, name, type)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值,name是新的名称
            ...
            //格式function(id, type)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值
            onItemFocus: null,
            //当双击节点时，触发的方法。格式function(id):id是节点的唯一表示ID
            onItemDClick: null,
            //当操作某个单元（结点/线）被由选中变成不选中时, 触发的方法, 返回FALSE可阻止取消选中事件的发生
            //格式function(id, type)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值
            onItemBlur: null,
            //当操作某个单元（结点/分组块）被重定义大小或造型时, 触发的方法, 返回FALSE可阻止重定大小/造型事件的发生
            //格式function(id, type, width, height)：id是单元的唯一标识ID,type是单元的种类,有"node","line"两种取值,
            // width是新的宽度,height是新的高度
            onItemResize: null,
            //当移动某条折线中段的位置, 触发的方法, 返回FALSE可阻止重定大小/造型事件的发生
            //格式function(id, M)：id是单元的唯一标识ID,M是中段的新X(或Y)的坐标
            onLineMove: null,
            ...
            onLineSetType: null,
            //当用重色标注某个结点/转换线时触发的方法, 返回FALSE可阻止重定大小/造型事件的发生
            //格式function(id, type, mark)：id是单元的唯一标识ID,type是单元类型（"node"结点,"line"转换线）, mark为布尔值,
            // 表示是要标注TRUE还是取消标注FALSE
            onItemMark: null,
            //ScubeUI 提示框
            showTipDialog: null
        },options;
