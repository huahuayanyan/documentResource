 ...
 */
var registerlistController = function($scope,$common,dataDeliver,$state,$eventCommunicationChannel) {
    ...
    $scope.startApply_onClick = function(){
        ...
        else
        {
            ...
            var data =
            {
                bCrrLnConCash:$scope.row
            }
            var promise = $common.get_asyncData("cashEqualValueRegisterService/ContractorQryGetter",data);
            promise.then(function(res){
                var lncon;
                if (res.errorCode =="SUC"){
                    ...
                    $common.get_asyncData("cashEqualValueRegisterService/findBPbcAppliBaseInfoByAppno", {appno:appno}).then(function (res) {
                        ...
                        if (res.errorCode == "SUC") {
                            ...
                            if(!applitask.piid)
                            {
                                ...
                                $state.go("registeredit");//跳转
                                window.source = $scope.source;
                            }
                            else
                            {
                                $common.get_asyncData("processService/getCurrentNodeEntity", {pid:taskform.piid}).then(function (res) {
                                    var taskform;
                                    if (res.errorCode == "SUC") {
