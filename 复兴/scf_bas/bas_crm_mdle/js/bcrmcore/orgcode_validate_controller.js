        ...
        if($.doValidate("otherform")){//校验数据格式
            ...
            primise.then(function(res){
                if (res.errorCode == "SUC"){
                    ...
                    if (data1.orgCode==dialogScope.otherform.orgCode && data1.coreFlag=="1" ){//查到的客户也是授信客户则提示该组织机构代码已经存在
                        $common.get_tipDialog("该组织机构代码已经存在，不可新增！",'showError');
                    }else if(data1.orgCode==dialogScope.otherform.orgCode && data1.coreFlag !="1" ) {//查到了客户，但是不是授信客户提示是否要转
                        var data= {
                            flag: "update",
                            bCrmBaseInfo:data1,
                            orgCode:data1.orgCode
                        };
                        $common.get_tipDialog('该用户已经存在，但并不是核心厂商，是否保存为核心厂商？','showTip').then(function(){//点击确定就直接会使用一下的功能
