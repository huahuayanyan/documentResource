 ...
 * Create at:   2016/5/18
 */
var contfreezeController = function($scope,$common,dataDeliver,$state,$eventCommunicationChannel){
  ...
  //  $scope.userInfo=userInfo;
    ...
    $scope.freeze_onClick = function(){   	
        if($scope.selected.select.length <= 0||$scope.selected.select.length > 1){
            $common.get_tipDialog('请选择一条记录！','showInform');
        }else{ 
        
            var promise = $common.get_asyncData("ibDcrLinService/freezeVbDcrLinByKey",{key:$scope.selected.select[0].bcCrId});
            promise.then(function(res){
                var data;
                if (res.errorCode =="SUC"){
                    $common.get_tipDialog('冻结成功','showInform');
                    queryTable();
    ...
    function queryTable(){
        var data= {
            ...
            vbDcrLin:{bcCrId:$scope.conditionForm.bcCrId,custNm:$scope.conditionForm.custNm,stat:'1'}
        };
