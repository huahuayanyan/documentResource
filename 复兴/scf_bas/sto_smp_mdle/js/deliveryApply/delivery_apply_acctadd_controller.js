...
var deliveryApplyAcctaddController = function($scope,$common,$state,dataDeliver) {
	$scope.acctinfo = {};
	$scope.source = window.source;
	$scope.baseinfo = $scope.source.baseinfo;
	$scope.baseinfo.operationType = "01";//现货静态提货
	...
	$common.get_asyncData("billNormalDeliveryApplyService/AppendBailAmountGetter",{deliveryQryVO:$scope.baseinfo}).then(function(res){
		...
		if (res.errorCode =="SUC"){
			...
			$scope.acctinfo = data;
			$scope.acctinfo.operationType = "01";//非标仓单提货
	...
	$scope.add_onClick=function(){
		$common.get_ngDialog('deliveryApply/info_loan_list.html',$scope,["$scope",function(dialogScope){
	...
	$scope.update_onClick=function(){
		if($scope.selected.select.length <= 0||$scope.selected.select.length > 1){
			...
			$common.get_ngDialog('deliveryApply/info_loan_edit.html',$scope,["$scope",function(dialogScope){
