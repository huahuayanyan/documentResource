...
var ReceMatlBindAddController = function(dialogScope,parentScope,$common,$state,dataDeliver) {
    ...
    if(productId != ""){
        ...
        //重置按钮  receMatlName，receMatlType
        dialogScope.rebort_onClick =function(){
            dialogScope.conditionForm.receMatlName = "";
            dialogScope.conditionForm.receMatlType = "";
            dialogScope.queryTable();
        }
        //点击查询之后的搜索
        ...
        dialogScope.queryTable = function(paramData){
            ...
            var promiseQ = $common.get_asyncData("bPrdReceMatlService/findBPrdReceMatlByPage",data);
        ...
        dialogScope.save_onClick = function(){
            if(dialogScope.selected.select.length > 0){
                   ...
                   var list = new Array();
                for (var i in arrs) {
                    var bPrdReceMatlBind={//后台需要的字段
                        productId:productId,
                        receMatlId:arrs[i].id
                    };
                    list[i] = bPrdReceMatlBind;
                }
                    var primiseS = $common.get_asyncData('bPrdReceMatlBindService/batchAdd',{bPrdReceMatlBindList:list})
                    primiseS.then(function(res){
                        if (res.errorCode =="SUC"){//新增成功之后，要将对应的数据显示在前台table上
                            parentScope.queryReceMatlTable();
