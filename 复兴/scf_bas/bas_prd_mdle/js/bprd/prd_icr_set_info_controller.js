...
var PrdIcrSetInfoController = function ($scope, $common, $state, dataDeliver) {
    ...
    if (productId != "") {//只有有产品号才可以进行一下的操作。
        ...
        $scope.queryTable = function (paramData) {
            ...
            var promise = $common.get_asyncData("bPrdCreditBindService/findBPrdCreditBindByPage", data);
        ...
        $scope.add_onClick = function () {
            ...
            $common.get_ngDialog('bprd/credit_bind_add.html', $scope
                , ["$scope", function (dialogScope) {
                    CreditBindAddController(dialogScope, $scope, $common, "add", row);
                }], "新增额度绑定信息", {showClose: true});
        };
        ...
        $scope.delete_onClick = function () {
            if ($scope.selected.select.length == 1) {
                $common.get_tipDialog('确定删除吗！', 'showTip').then(function () {
                    ...
                    var primiseA = $common.get_asyncData('bPrdCreditBindService/delete', {id: id})
        ...
        $scope.update_onClick = function () {
            ...
            if ($scope.selected.select.length == 1) {
                $common.get_ngDialog('bprd/credit_bind_add.html', $scope
                    , ["$scope", function (dialogScope) {
                        CreditBindAddController(dialogScope, $scope, $common, "update", row);
                    }], "修改额度绑定信息", {showClose: true});
            } else {
                $common.get_tipDialog('请选择一条记录！', 'showInform');
            }
        };
        ...
        $scope.column = function (row) {
            $common.get_ngDialog('bprd/credit_bind_add.html', $scope
                , ["$scope", function (dialogScope) {
                    CreditBindAddController(dialogScope, $scope, $common, "view", row);
                }], "查看额度绑定信息", {showClose: true});
        }
       ...
       //-----------------点击下一步-----------------
        $scope.next_onClick = function () {
            $state.go("tree.prdCopLibBind");
            $scope.$parent.panes.forEach(function(pane){
                if (pane.uiSref == "tree.prdCopLibBind"){
                    $scope.$parent.select(pane);
                };
            });
        }
