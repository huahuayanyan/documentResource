...
var CustomAsynchTree = function() {
	...
	this.initParameter = function(tData) {
		...
		this.queryNodeFun = tData['queryNodeFun'];
	...
	this.getNodeHtml = function(fatherId) {
		...
		this.removeNode(fatherId);
		// STEP-2 异步查询子节点
		var nodes = this.queryChildNode(fatherId);
		if(null != nodes) {
			for (var i = 0 ; i < nodes.length ; i++ ) {
				var everyChild = nodes[i];
	 ...
	 * 异步根据nId查询子节点
	 */	
	this.queryChildNode = function(fatherId) {
		if(this.queryNodeFun) {
			return this.queryNodeFun(fatherId);
		} else {
			return null;
		}
	}
	...
	this.pushNode = function(nodes) {
		if(nodes && null != nodes) {
			for (var i = 0 ; i < nodes.length ; i++ ) {
				this.allChildNode.push(nodes[i]);
			}
		}
	}
	 ...
	 * 根据fId删除容器中子节点
	 */	
	this.removeNode = function(fId) {
		for (var i = 0 ; i < this.allChildNode.length ; i++ ) {
			var everyChild = this.allChildNode[i];
			if (fId+'' == everyChild['fId']+'' ) {
