...
var nrecBcpImpairmentListController = function($scope,$common,$state,dataDeliver) {
    $scope.conditionForm={//申请类型 3229 应收账款池融资折让,3203 应收账款折让申请(国内保理),3230 信保保理折让,3231 双保理折让,3232 租赁保理折让
        appliType:"3229,3203,3230,3231,3232"
    };
    ...
    $scope.$on('interceptEvent',function(event,data) {
        ...
        $common.get_asyncData('ndebtImpairmentService/submit',{appNo:$scope.selected.select[0].appno}).then(function(res){
            if (res.errorCode =="SUC"){
                $scope.queryTable();
                $scope.selected.select=[];
