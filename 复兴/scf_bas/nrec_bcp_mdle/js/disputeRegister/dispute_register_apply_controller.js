 ...
 */
var dRegisterApplyController = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
    ...
    function factTypeJudge(){
        ...
        var promiseA = $common.get_asyncData("disputeRegisterService/findRBcpAppliBussInfoByAppno",data);
        promiseA.then(function(res1){
            if(res1.errorCode =="SUC") {
        ...
        $common.get_ngDialog(url, $scope
            ,["$scope",function(dialogScope){
                ...
                dialogScope.queryTable = function(paramData){
                    ...
                    var promise = $common.get_asyncData("rbcpDebtInfoService/findRBcpDebtInfoForPRegister",data);
                    promise.then(function(res){
                        var data;
    ...
    $scope.editDispute_onClick = function(){
        if($scope.bcpSelected.select.length != 1){
            ...
            var url;
            if(window.appNo){
                url='../../nrec_bcp_mdle/html/disputeRegister/dispute_info_edit.html'
            }else{
                url='disputeRegister/dispute_info_edit.html';
            }
            $common.get_ngDialog(url, $scope
                ,["$scope",function(dialogScope){
                    ...
                    if(dialogScope.otherform.issueReason=="14"){//争议金额为其他的时候其他原因显示出来
                    ...
                    function checkMoney(){
                        ...
                        if(money == 0 ){
                            $common.get_tipDialog('争议金额需大于0！','showError');
                            return false;
                        }
                        if(money > remainingAmount){
                            $common.get_tipDialog('争议金额不可大于应收账款余额！','showError');
                            return false;
                        }
                        return true;
                    }
   ...
   //删除票据信息
    ...
    function checkBcpdatasource(){
        ...
        for(var x in bcpDatasource){
            if(!(bcpDatasource[x].money)||bcpDatasource[x].money==""||bcpDatasource[x].money==0 ){
                $common.get_tipDialog(("请填写凭证编号为"+bcpDatasource[x].billsNo+"的单据争议金额！"), 'showError');
                return false;
            }
            if(!(bcpDatasource[x].issueReason)|| bcpDatasource[x].issueReason==""){
                $common.get_tipDialog(("请填写凭证编号为"+bcpDatasource[x].billsNo+"的单据争议原因！"), 'showError');
                return false;
            }
        }
        return true;
    ...
    $scope.save_onClick=function(){
        if($.doValidate("otherform")) {
            if(checkBcpdatasource()==true){
                ...
                var promise = $common.get_asyncData("disputeRegisterService/saveOrUpdateDisputeRegisterInfo", data2);
                promise.then(function (res) {
                    if (res.errorCode == "SUC") {
    ...
    $scope.$on('interceptEvent',function(event,data) {
        ...
        var promise = $common.get_asyncData("disputeRegisterService/submitDisputeRegisterInfo", data2);
        promise.then(function (res) {
            if (res.errorCode == "SUC") {
