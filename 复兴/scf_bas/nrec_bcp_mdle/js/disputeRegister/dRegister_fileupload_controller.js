 ...
 */
var disputeRegisterFileUploadController = function($scope,$common,dataDeliver,$state,$eventCommunicationChannel) {
    if(window.appNo) {
        $scope.hide1 = true;//返回按钮
    }
    var dataX=dataDeliver.getter();
    $scope.otherform = dataDeliver.getter();//内含申请号，合同号和flag
    if(!dataX.appno) {
    ...
    //监听分页
    $scope.$watch("fileDatasource",function(){
        $scope.datasource.ds=[];
        $scope.conf.totalRecord=$scope.fileDatasource.length;
        if($scope.fileDatasource.length > 0){
            var pageNo=parseFloat($scope.conf.pageNo);
            var pageSize=parseFloat($scope.conf.pageSize);
            var y ;
            if(pageNo*pageSize>=$scope.conf.totalRecord){
                y=$scope.conf.totalRecord-1;
            }else{
                y=pageNo*pageSize-1;
            }
            for(var x =(pageNo-1)*pageSize;x<=y;x++){
                $scope.datasource.ds.push($scope.fileDatasource[x]);
            }
        }
    });
