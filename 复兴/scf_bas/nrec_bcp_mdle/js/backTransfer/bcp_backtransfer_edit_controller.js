 ...
 */
var nrecBcpBackTransferEditController = function($scope,$common,dataDeliver,$state,$eventCommunicationChannel) {
    ...
    //            var totalAmount = 0;
    //            var debetNo = "";//存放应收账款ID
    //            for(var i=0;i<$scope.datasourcedebt.ds.length;i++)
    //            {
    //                if($scope.datasourcedebt.ds[i].id)
    //                {
    //                    debetNo = debetNo + $scope.datasourcedebt.ds[i].id+"/f";
    //                }
    ...
    $scope.$watch("conflnci.pageNo+conflnci.pageSize",function(){
        var debetNo = $scope.otherform.debetNo;
        if(debetNo!=null&&debetNo!=""&&debetNo!=undefined)
        {
            $scope.queryTableLnci();
        }
    });
    ...
    $scope.queryTableLnci = function(paramData)
    {
        ...
        else
        {
            ...
            $common.get_asyncData("nfactoringDebtBackService/getLanLnciInfoByContno", data).then(function (res) {
                var data;
                if (res.errorCode == "SUC") {
                    data = res.contextData;
    ...
    $scope.add_onClick = function() {
        if($.doValidate("otherform")){
            $common.get_ngDialog('backTransfer/bcp_debtdtl.html', $scope
                ,["$scope",function(dialogScope){
                    ...
                    dialogScope.queryTable = function(paramData){
                        ...
                        var promise = $common.get_asyncData("nfactoringDebtBackService/InvoiceDebtBackAddQryGetter",data);
                    ...
                    dialogScope.save_onClick = function(){
                        if(dialogScope.selected.select.length <= 0){
                            ...
                            for(var k in selected)
                            {
                                ...
                                if(selected[k].id)
                                {
                                    debetNo = debetNo + selected[k].id+"/f";
                                }
        ...
        if($scope.selecteddebt.select.length == 0){
            ...
            for (var i = 0; i < dsArray.length; i++)
            {
                for(var x in selected)
                {
                    ...
                    {
                        debetNo = debetNo + dsArray[i].debtId+"/f";
                    }
                }
            }
            $scope.otherform.debetNo = debetNo;
            ...
            $common.get_asyncData("nRBcpAppliBussDtlService/queryRBcpAppliBussDtlByAppno", {appno:appno}).then(function (res) {
                ...
                if (res.errorCode == "SUC") {
                    ...
                    for(var i=0;i<debtList.length;i++)
                    {
                        ...
                        debetNo = debetNo + debtList[i].debtId+"/f";
                    }
                    if(totalAmount==0)
                    {
                        $scope.otherform.totalAmount = "0.00";
