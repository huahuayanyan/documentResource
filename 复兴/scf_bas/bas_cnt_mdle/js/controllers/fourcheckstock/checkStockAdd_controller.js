...
var checkStockAddController = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
    ...
    $scope.detail_onClick = function(){
        if($scope.selected.select.length != 1){
            ...
            $common.get_ngDialog('fourcheckstock/threeProtocDtl.html', $scope, ["$scope", function (dialogScope) {
                ...
                function queryDebt(){
                    ...
                    var promise = $common.get_asyncData("fourCheckStockService/fourCheckStockDebetByPage", data);
                    promise.then(function (res) {
                        var data;
                        if (res.errorCode == "SUC") {
                            data = res.contextData;
                    ...
                    var promise_conditionForm = $common.get_asyncData("fourCheckStockService/fourCheckStockDebtSum", data);
                    //BCntFourCheckStockServiceImpl
                    promise_conditionForm.then(function (res) {
                        var data;
                        if (res.errorCode == "SUC") {
                            data = res.contextData;
                            dialogScope.lnciAmtSum = data.lnciAmtSum;
                            dialogScope.amountOutSum = data.amountOutSum;
                            dialogScope.amountInSum = data.amountInSum;
                        } else {
                            $common.get_tipDialog(res.errorMsg, 'showError');
                        }
                    });
    ...
    $scope.applyCheck_onClick = function() {
        if ($scope.selected.select.length != 1) {
            ...
            $common.get_tipDialog('是否发起对账？','showTip').then(function(){
                ...
                var promise = $common.get_asyncData("fourCheckStockService/fourCheckStockDebt", param);
                promise.then(function (res) {
                    if (res.errorCode == "SUC") {
                        ...
                        if(debetData.length>0){
                            ...
                            var primise = $common.get_asyncData("fourCheckStockService/fourCheckStockApply", data);
