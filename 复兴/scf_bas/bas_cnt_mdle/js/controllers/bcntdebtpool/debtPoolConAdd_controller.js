...
var debtPoolConAddController = function($scope,$common,dataDeliver,$state,$compile,$eventCommunicationChannel) {
...
//费用收取方式
  ...
  $scope.debtExtconf= {//初始化分页对象交易对手
    ...
    $scope.debtExtupdate_onClick = function(){
     ...
     $common.get_ngDialog('bcntdebtpool/debtExtMod.html',$scope
     ,["$scope",function(dialogScope){
    // bicrinfosubController(dialogScope,$scope,$common,"update");
    dialogScope.debtExtform=$common.copy($scope.debtExtselected.select[0]);
        //有无追索权
             //池有无追索权 账期，融资比率 ，宽限期都为必输 不根据有无追索权 来区别
  /*       if(dialogScope.debtExtform.recoverType==1){
             $scope.creditTermDisable=true;
             $scope.creditAmtDisable=true;
             $scope.riskTermDisable=true;
         }/!*else{
                 $scope.creditTermDisable=false;
                 $scope.creditAmtDisable=false;
                 $scope.riskTermDisable=false;
             }*!/
