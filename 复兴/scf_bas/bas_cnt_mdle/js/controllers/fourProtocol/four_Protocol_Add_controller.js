...
var bcntFourProtocolAddController = function($scope,$common,dataDeliver,$state,$compile,$eventCommunicationChannel) {
    ...
    function findthreeProtocolFormInfo(){
        var data= {
            pageNo: 1,
            pageSize: 10,
            deliveryQryVO:{protocolNo:dataX.protocol.protocolNo}
        };
    ...
    $scope.$watch("threeProtocolForm.cooperationBrcodeNm",function(){
        if(!$scope.threeProtocolForm.cooperationBrcodeNm || $scope.threeProtocolForm.cooperationBrcodeNm == ''){
            $scope.threeProtocolForm.cooperationBrcodeNm = null;
        }
        //ht-input-extent  需要用监听的方式来传额外的参数
