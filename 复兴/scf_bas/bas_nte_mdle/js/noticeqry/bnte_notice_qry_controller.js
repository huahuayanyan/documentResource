/**
 * Created by htrad0024 on 2017/1/4.
 */
//收款确认通知书
var BnteNoticeQryController = function ($scope, $common, $state, dataDeliver) {
    $scope.conditionForm = {
        productName: "07"
    };
    ...
    $scope.selectSource = {
        ds:[
            {key:"75",value:'收款确认(流贷)通知书'},
            {key:"78",value:'收款确认(银承)通知书'}
        ]
    };
    ...
    $scope.queryTable = function (paramData) {
        ...
        var promise = $common.get_asyncData("bNteNoticeQryVOService/queryNoticeQryVOByPage", data);
...
//查看操作！
    $scope.column = function(row){
        $common.get_ngDialog('noticeqry/bnte_notice_qry_view.html', $scope
            ,["$scope",function(dialogScope){
                BnteNoticeQryViewController(dialogScope,$scope,$common,row);
            }],"收款确认通知书基本信息",{showClose:true});
    };
    //重置按钮
    $scope.rebort_onClick = function () {
        $scope.$apply(function(){
            $scope.conditionForm = {productName: "07"};
