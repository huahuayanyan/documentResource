...
var debtPoolAawinstNoticeController = function($scope,$common,$state,dataDeliver) {
    ...
    $scope.queryTable = function(paramData){
        if($scope.conditionForm.noticeType==null||$scope.conditionForm.noticeType=='') {
            $scope.conditionForm.noticeType = '101';//应收账款转让/出质通知书
        }
    ...
    $scope.rebort_onClick =function(){
        ...
        $scope.$apply(function(){
            $scope.conditionForm = {noticeType:"101"};
            $scope.selected.select=[];//清空选项s
        })
    };
    //查看操作！
    $scope.column = function(row){
        $common.get_ngDialog('debtPoolAawinstNotice/debtPoolAawinsNoticeDtl.html', $scope
            ,["$scope",function(dialogScope){
                debtPoolAawinsNoticeDtlController(dialogScope,$scope,$common,row);
            }],"应收账款转让/出质通知书基本信息",{showClose:true});
    };
