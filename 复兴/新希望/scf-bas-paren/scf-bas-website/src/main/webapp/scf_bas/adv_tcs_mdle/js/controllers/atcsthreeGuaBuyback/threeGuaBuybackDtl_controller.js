...
var threeGuaBuybackDtlController = function($scope,$common,dataDeliver,$state,$compile,$eventCommunicationChannel) {
    $scope.threeGuaBuybackform={};
    ...
    $scope.coreNmdooextent = function(ds){
        $scope.threeGuaBuybackform.coreCustcd=ds.custcd;
        $scope.threeGuaBuybackform.coreNm=ds.cname;
    };
    $scope.$watch("threeGuaBuybackform.coreNm",function(){
        //ht-input-extent  需要用监听的方式来传额外的参数
        $scope.coreNmContent.param ={
            coreFlag:"1", //核心厂商
            cname:$scope.threeGuaBuybackform.cname
    ...
    $scope.dooextent = function(ds){
        $scope.threeGuaBuybackform.custcd=ds.custcd;
        $scope.threeGuaBuybackform.sellerNm=ds.cname;
    };
    $scope.$watch("threeGuaBuybackform.sellerNm",function(){
        //ht-input-extent  需要用监听的方式来传额外的参数
        $scope.sellerNmContent.param ={
            creditFlag:'1',
            custType:'1',
            sellerNm:$scope.threeGuaBuybackform.cname,
            custcd:$scope.threeGuaBuybackform.custcd
        }
    });
    //买方客户名称下拉-end
    ...
    $scope.fouNameExtent = function(ds){
        $scope.threeGuaBuybackform.fouCd=ds.custcd;
        $scope.threeGuaBuybackform.fouName=ds.cname;
    };
    $scope.$watch("threeGuaBuybackform.coreNm",function(){
        //ht-input-extent  需要用监听的方式来传额外的参数
        $scope.fouNameContent.param ={
            coreFlag:'1',
            custType:'1',
            fouName:$scope.threeGuaBuybackform.fouName,
            fouCd:$scope.threeGuaBuybackform.fouCd
        }
    });
    //承担方客户名称下拉-end
    ...
    function queryProtocol(){
        ...
        promise.then(function (res) {
            if(res.errorCode=="SUC"){
                data = res.contextData;
                $scope.threeGuaBuybackform=data;
            }else{
                $common.get_tipDialog(res.errorMsg,'showError');
            }
        })
    ...
    $scope.save_onClick = function(){
        if($.doValidate("otherform")) {
            ...
            var data = {
                threeBalanceRefundVO: $scope.threeGuaBuybackform,
            ...
            primise.then(function (res) {
                if (res.errorCode == "SUC") {
                    data = res.contextData;
                    $scope.threeGuaBuybackform.appno=data.value;
                    $scope.sumbitdisable= false;
                    //$scope.queryTable();
                    //$scope.selected.select = [];
                    var appno={
                    		appno:$scope.threeGuaBuybackform.appno
                        };
