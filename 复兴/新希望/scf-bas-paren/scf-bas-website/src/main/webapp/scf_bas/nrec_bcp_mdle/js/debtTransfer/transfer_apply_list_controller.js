...
var TransferAppplyListController = function($scope,$common,$state,dataDeliver) {
    //console.log("userInfo信息："+JSON.stringify(userInfo));//用户信息，全局变量
    /*alert("userInfo信息："+JSON.stringify(userInfo));*/
    $scope.conditionForm={//申请类型 3284，应收账款池转让申请，3201 应收账款转让申请,3249 国内双保理转让,3248 信保保理转让申请,3250 租赁保理转让申请
        appliType:"3201,3287,3249,3248,3250"
    };
    ...
    $scope.$on('interceptEvent',function(event,data) {
        ...
        var promise = $common.get_asyncData("transferApplyService/submitTransferApply",{key:appno});
        promise.then(function(res){
            if (res.errorCode =="SUC"){//数据中的经办机构要转换成中文
                $scope.selected.select=[];
                $scope.flowsubmitdisable = false;
    ...
    //通知书查看
    $scope.showNotice_onClick = function()
    {
        if($scope.selected.select.length != 1){
            $common.get_tipDialog('请至少选择一条记录！','showInform');
        }else{
            var row = $scope.selected.select[0];
            if(row.appliStatus!="02")
            {
                $common.get_tipDialog('审批未通过，未生成通知书信息！','showInform');
                return;
            }
            //通过appno获取催收方式
            $common.get_asyncData("nRBcpAppliBussInfoService/findBcpAppliBussInfoByAppno",{key:row.appno}).then(function(res){
                if (res.errorCode =="SUC"){//数据中的经办机构要转换成中文
                    var data = res.contextData;
                    var param = {
