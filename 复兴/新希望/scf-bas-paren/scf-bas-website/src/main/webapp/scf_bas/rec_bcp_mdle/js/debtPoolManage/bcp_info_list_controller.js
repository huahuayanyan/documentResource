...
var BcpInfoListController = function($scope,$common,$state,dataDeliver) {
    ...
    $scope.conditionForm={//需要先定义业务品种对应的产品ID
        bussType:"1230040,1230050,1230060",//后台对应的业务品种字段
        flag:"01"
    };
   ...
   // billsType，billsNo，status
    ...
    $scope.add_onClick = function(){
        var row={};
        $common.get_ngDialog('debtPoolManage/buss_choose.html', $scope
    ...
    //应收账款信息删除操作（基本信息，明细信息删除）
    $scope.delete_onClick = function(){
        if($scope.selected.select.length == 1){
            if($scope.selected.select[0].status == "01"){//只有已录入的可以删除
                if($scope.selected.select[0].isLocked == "0"){//只有未锁定的可以删
