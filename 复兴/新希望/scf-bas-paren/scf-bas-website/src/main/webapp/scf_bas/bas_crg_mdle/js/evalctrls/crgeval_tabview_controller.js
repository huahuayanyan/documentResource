 ...
 */
var tabviewController = function($scope,$common,dataDeliver,$state,$eventCommunicationChannel) {
    ...
    function bcrgEvalToValue(){
	    var promise = $common.get_asyncData("iBCrgEvalRecService/findExtBCrgEvalByKey",{key:data.extBCrgEval.evalId});
    ...
    function init(){
        ...
        $scope.confuse_onClick=function(){
        	//经营分析,客户信用视图 查看信用返回到经营分析,客户信用视图 
        	if($state.current.name=="custCrgevalView"){
                $state.go("business");
            }else{
            	 $state.go("list");
            }
