...
var fourConfirmDtlController = function($scope,$common,dataDeliver,$state,$compile,$eventCommunicationChannel) {
    $scope.otherform={};
    $scope.fourConfirmform={};
...
//根据合同编号加载合同基本信息
    ...
    function queryDebtInfo(){
        ...
        promise.then(function (res) {
            if(res.errorCode=="SUC"){
                data = res.contextData;
               // $scope.debtform=data;
                $.extend($scope.fourConfirmform,res.contextData); //合并
            }else{
                $common.get_tipDialog(res.errorMsg,'showError');
            }
        })
    ...
    $scope.coreNmdooextent = function(ds){
        $scope.fourConfirmform.coreCustcd=ds.custcd;
        $scope.fourConfirmform.coreNm=ds.cname;
    };
    $scope.$watch("fourConfirmform.coreNm",function(){
        //ht-input-extent  需要用监听的方式来传额外的参数
        $scope.coreNmContent.param ={
            coreFlag:"1", //核心厂商
            cname:$scope.fourConfirmform.cname
...
//买方名称
    ...
    $scope.dooextent = function(ds){
        $scope.fourConfirmform.custcd=ds.custcd;
        $scope.fourConfirmform.cname=ds.cname;
    };
    $scope.$watch("fourConfirmform.cname",function(){
        //ht-input-extent  需要用监听的方式来传额外的参数
    ...
    $scope.productNamedooextent = function(ds){
        $scope.fourConfirmform.productName=ds.productName;
        $scope.fourConfirmform.productId=ds.productId; //业务品种
        $scope.fourConfirmform.factType=ds.factType;
        $scope.fourConfirmform.financingType=ds.financingType;
    };
    $scope.$watch("fourConfirmform.productName",function(newVal,oldVal){
        //ht-input-extent  需要用监听的方式来传额外的参数
        $scope.productNameContent.param ={
            productName:$scope.fourConfirmform.productName,
...
//购销合同查看
    $scope.column = function(row){
        ...
        $common.get_ngDialog('abcpfourConfirm/contTradeInfoView.html',$scope
    ...
    $scope.save_onClick = function(){
        ...
        $scope.savedisable = true;
            var data = {
                threeConfirmVO: $scope.fourConfirmform
            };
