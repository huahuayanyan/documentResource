...
var dsptRemAddController = function($scope,$common,dataDeliver,$state,$stateParams,$compile,$eventCommunicationChannel) {
    ...
    if(!window.appNo){
        // ext2 stratDate
        $scope.otherform.ext2="应收账款池融资减值申请";
...
// add,update,del,progress,save,submit,back
    ...
    $scope.save_onClick= function(){
        if($.doValidate("cntDebtInfoForm")){//实际上这里还需要再去后台检验下客户名称和客户号是否一致 todo!!!!
            ...
            var processModel = {
                modelId: 'AllowancePoolApply',
                ProcessName: '应收账款池融资减值申请',
                NodeId: 'Write',
        ...
        if($.doValidate("cntDebtInfoForm")){//实际上这里还需要再去后台检验下客户名称和客户号是否一致 todo!!!!
            ...
            primiseX.then(function (res) {
                if (res.errorCode == "SUC") {
                    ...
                    if ($state.current.name == "bsystasktab.AllowancePoolApply") {
                        $state.go("bsystask");
                    } else {
                        $state.go("impairPoolConList");
                    }
