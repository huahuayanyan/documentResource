 ...
 * Create at:   2016/9/29
 */
var DebtInfoEditController =  function($scope,$common,$state,dataDeliver){
    var data1=dataDeliver.getter();
    $scope.otherform=data1
    ...
    $scope.save_onClick=function(){
            if($.doValidate("otherform")){
                ...
                if(errorflag){
                    ...
                    var primise = $common.get_asyncData('rbcpDebtInfoService/updateRBcpDebtInfo',{rBcpDebtInfoVO:$scope.otherform})
                    primise.then(function(res){
                        if (res.errorCode =="SUC"){
                            $common.get_tipDialog('修改成功！', 'showSuccess');
                            var url = "bcpInfoList";
                            if(flag=="contupdate"||flag=="contview")
                            {
                                url = "cont.debt";
                                var baseinfo = data1.baseinfo;
                                dataDeliver.setter(baseinfo);
                            }
                            $state.go(url);
                        }else{
