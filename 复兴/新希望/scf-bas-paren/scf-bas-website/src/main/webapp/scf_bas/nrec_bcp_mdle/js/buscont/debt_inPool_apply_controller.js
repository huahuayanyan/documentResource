 ...
 */
var debtInPoolApplyController = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
        ...
        $common.get_ngDialog(url, $scope
            ,["$scope",function(dialogScope){
                ...
                dialogScope.rebort_onClick =function(){
                    dialogScope.conditionForm={//需要先定义业务品种对应的产品ID
                        mastContno:mastContno,//业务合同号
                        lockAppno:appno ,poolFlag:'0'//出池的
                    };
    ...
    $scope.save_onClick=function(){
        if($.doValidate("otherform")) {
            if(checkBcpdatasource()==true){
                ...
                var promise = $common.get_asyncData("debtInPoolService/saveOrUpdateInPoolInfo", data2);
                promise.then(function (res) {
                    if (res.errorCode == "SUC") {
    ...
    $scope.submit_onClick=function(){
        if($.doValidate("otherform")) {
            if(checkBcpdatasource()==true){
                ...
                var promise = $common.get_asyncData("debtInPoolService/submitInPoolInfo", data2);
                promise.then(function (res) {
                    if (res.errorCode == "SUC") {
