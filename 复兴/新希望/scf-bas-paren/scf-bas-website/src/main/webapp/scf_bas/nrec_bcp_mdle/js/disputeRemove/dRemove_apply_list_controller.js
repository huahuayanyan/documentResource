...
var dRemoveApplyListController = function($scope,$common,$state,dataDeliver) {
    //console.log("userInfo信息："+JSON.stringify(userInfo));//用户信息，全局变量
    /*alert("userInfo信息："+JSON.stringify(userInfo));*/
    $scope.conditionForm={//申请类型 3218 商业纠纷解除申请,3237 应收账款池融资商纠解除,3239 双保理商纠解除,3238 信保保理商纠解除,3240 租赁保理商纠解除
        appliType:"3218,3237,3239,3238,3240"
    };
            ...
            $common.get_asyncData("bprdChkdirInfoService/findBPrdChkdirInfoByParam",{omap:param}).then(function(res){
                if (res.errorCode =="SUC"){
                    ...
                    if(mdlId==undefined||mdlId==""||mdlId==null)
                    {
                        ...
                        var appno=row.appno;
                        var promise = $common.get_asyncData("disputeRemoveService/submitDisputeRemoveInfoApply",{key:appno});
                        promise.then(function(res){
                            if (res.errorCode =="SUC"){//数据中的经办机构要转换成中文
