...
//押品流水查询
var goodsInHouseController = function ($scope, $common, $state, dataDeliver) {
    ...
    $scope.rebort_onClick = function () {
        $scope.$apply(function(){
            $scope.conditionForm = {supplyChainPdId : '1220020'};
        })
    };
    ...
    $scope.dtlView_onClick = function() {
        if ($scope.selected.select.length != 1) {
            ...
            $common.get_ngDialog('goodsInHouse/goodsInHouseDtl.html', $scope
                ,["$scope",function(dialogScope){
                    goodsInHouseDtlController(dialogScope,$scope,$common,"view",$scope.selected.select[0]);
                }],"",{showClose:true});
    ...
    $scope.levelTwoContent = {
        type:"table",//展示类型table  tree
        url:"bPbcMtgClassService/findBPbcMtgClassForTwoLevelSelect",
        param:{levels:"2",associateOne :false},
        //split:"",//分隔符
        extentColumn:"mortgageName",
        column:["mortgageName"]//展示内容
    };
