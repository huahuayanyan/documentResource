 ...
 */
var multiPriceAdjustControllerXhjt = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
    //任务基本信息初始化
    $scope.taskForm = {
        ext2:"现货质押批量价格调整申请(静态)",
    };
    ...
    if(window.appNo){//如果有申请号
        ...
        promiseB.then(function(res){
            if (res.errorCode =="SUC") {
                ...
                promise.then(function(res){
                    if (res.errorCode =="SUC"){
                        $scope.taskForm.ext2="现货质押批量价格调整申请(静态)";
        ...
        promise.then(function(res){
            if (res.errorCode =="SUC"){
                ...
                $scope.applyInfo.productId="1220010";
            }else{
                $common.get_tipDialog(res.errorMsg,'showError');
            }
        });
    ...
    if(!window.appNo){
        $scope.applyInfo={applyType:"2",productId:"1220010"}
        $scope.submit=true;
    }
    //押品下拉菜单
    // 押品一级
