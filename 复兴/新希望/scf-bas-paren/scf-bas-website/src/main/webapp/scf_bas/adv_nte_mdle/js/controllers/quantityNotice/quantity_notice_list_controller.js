...
//质物价格确定/调整通知书列表
var QuantityNoticeListController = function ($scope, $common, $state, dataDeliver) {
...
//查看操作！
    $scope.column = function(row){
        $common.get_ngDialog('quantityNotice/quantity_notice_view_qry.html', $scope
            ,["$scope",function(dialogScope){
                QuantityNoticeViewController(dialogScope,$scope,$common,row);
            }],"质物价格调整通知书基本信息",{showClose:true});
    };

    //重置按钮
    $scope.rebort_onClick = function () {
        $scope.$apply(function(){
            $scope.conditionForm = {bussType:'1210020', noticeType:'0'};
