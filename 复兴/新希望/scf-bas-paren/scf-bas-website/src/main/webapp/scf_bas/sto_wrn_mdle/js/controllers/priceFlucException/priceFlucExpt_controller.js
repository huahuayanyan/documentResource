...
//押品流水查询
var PriceFlucExptController = function ($scope, $common, $state, dataDeliver) {

    $scope.conditionForm = {
    	//warType=1&warnNo=122001004	
        warnType_Q:'1',
        warnNo_Q:'122001004'
    };//初始化查询条件对象
    ...
    $scope.queryTable = function (paramData) {
        ...
        if(fluctuation1_Q>fluctuation2_Q){
           $common.get_tipDialog("请确保波动率查询范围 由小到大","showInform");
           return;
        }
        if(fluctuation1_Q<0){
            $common.get_tipDialog("波动率范围(%)必须为0-100","showInform");
            return;
        }
    ...
    $scope.rebort_onClick = function () {
        $scope.$apply(function(){
            $scope.conditionForm = {};
            $scope.selected.select=[];
            $scope.conditionForm.warnType_Q='1';
            $scope.conditionForm.warnNo_Q='122001004';
        })
        $scope.queryTable();
    };
