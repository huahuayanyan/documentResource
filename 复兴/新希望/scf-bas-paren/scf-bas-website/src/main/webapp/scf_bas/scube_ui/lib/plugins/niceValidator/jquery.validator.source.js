...
(function(factory) {
    ...
    var NS = 'validator',
        ...
        defaults = {
            ...
            beforeSubmit: noop,
            //dataFilter: null,
            valid: function(){//默认改成ajax提交，这样不会出现全局刷新
            },
            invalid: function(){
            },
    ...
    Validator.prototype = {
        ...
        showMsg: function(el, msgOpt, /*INTERNAL*/ field) {
            ...
            var me = this,
                ...
                temp,
                $msgbox;
            /**xiangsl 增加float四种主题提示**/
            if(opt.theme.indexOf('float')>=0){
            	var position=opt.theme.replace('float-','');
            	$(el).floatRemove();
        		$(el).popFloat({
        	        targetMode: "remind",
        	        target: msgOpt.msg,
        	        position:position,
        	        bindType : "click",
        	        bindCall : function(parent){
        	        	$(parent).floatRemove();
        	        }
        		});
        		return;
            }
            /**xiangsl 增加float四种主题提示 end**/
        ...
        hideMsg: function(el, msgOpt, /*INTERNAL*/ field) {
            var me = this,
                opt = me.options,
                $msgbox;
            /**xiangsl 增加float四种主题隐藏**/
            if(opt.theme.indexOf('float')>=0){
            	$(el).floatRemove();
        		return;
            }
            /**xiangsl 增加float四种主题隐藏 end**/
            el = $(el).get(0);
            msgOpt = me._getMsgOpt(msgOpt);
            if ($(el).is(INPUT_SELECTOR)) {
                field = field || me.getField(el);
