 ...
 */
@ScubeService
@Service("BNteNoticeBaseServiceImpl")
public class BNteNoticeBaseServiceImpl implements IBNteNoticeBaseService {
	...
	@Resource(name = IBCntDebtExtInfoService.BEAN_ID)
	IBCntDebtExtInfoService bCntDebtExtInfoService; // 保理合同交易对手信息
...
//	ISBcpAppliLowerPriceService sBcpAppliLowerPriceService;
	...
	public BNteNoticeBase findBNteNoticeBaseByKey(String key) throws ScubeBizException {
		...
		try {
			BeanUtils.copyProperties(bNteNoticeBasedal, bNteNoticeBase);
		} catch (BeansException e) {
			log.error("通知书基本信息转换异常！");
			e.printStackTrace();
			throw new ScubeBizException(ScfMessageUtil.transErrMsgByKey(BNteErrorConstant.SCF_BAS_NTE_10000), BNteErrorConstant.SCF_BAS_NTE_10000);
	...
	public void savePromptPaymentNoticeInfo(String appno) throws ScubeBizException {
		...
		if (StringUtil.isStrNotEmpty(bPbcAppliBaseInfo.getMastContno())) {
			bCntDebtInfo = bCntDebtInfoService.findBCntDebtInfoByKey(bPbcAppliBaseInfo.getMastContno());
		...
		if (ScfBasConstant.FACT_TYPE_INLAND.equals(factType) || ScfBasConstant.FACT_TYPE_REVERSE.equals(factType)
			...
			bean.setNoticeType(BNteConstant.NOTICE_TYPE_116);
			...
			bean.setNoticeType(BNteConstant.NOTICE_TYPE_117);
			 ...
			 * bean.setNoticeType(BNteConstant.NOTICE_TYPE_118); }else
			 * if(ScfBasConstant.FACT_TYPE_CREDIT_FACT.equals(factType)){
			 * bean.setNoticeType(BNteConstant.NOTICE_TYPE_119); }else
			 * if(ScfBasConstant.FACT_TYPE_LEASE_FACT.equals(factType)){
			 * bean.setNoticeType(BNteConstant.NOTICE_TYPE_120); }else
			 * if(ScfBasConstant.FACT_TYPE_DEBT_PLEDGE.equals(factType)){
			 * bean.setNoticeType(BNteConstant.NOTICE_TYPE_121); }
			 */
		...
		bean.setStatus(BNteConstant.NOTICE_BASE_STATUS_UNCONFIRM);
	...
	public void dealWithNoticeInfo(String appno) throws ScubeBizException {
		...
		// String noticeNum = getNoticeNo();
		// bean.setId(this.getUUID());
		// bean.setNum(String.valueOf(noticeNum));
		// bean.setAdviceid(String.valueOf(noticeNum));
		bean.setAppno(bPbcAppliBaseInfo.getAppno());
		bean.setMastContno(bPbcAppliBaseInfo.getMastContno());
		bean.setCustcd(bPbcAppliBaseInfo.getCustcd());
		BCntDebtInfo bCntDebtInfo = null;
		...
		if ((WorkflowConstant.APPLI_TYPE_DISPUTEINVOICESETAPPLY).equals(applyType)) {// 商业纠纷设定申请
			bean.setNoticeType(BNteConstant.NOTICE_TYPE_DISPUTEINVOICESETAPPLY);
		} else if ((WorkflowConstant.APPLI_TYPE_DISPUTEINVOICREMOVEAPPLY).equals(applyType)) {// 商业纠纷解除申请
			bean.setNoticeType(BNteConstant.NOTICE_TYPE_DISPUTEINVOICREMOVEAPPLY);
		} else if ((WorkflowConstant.APPLI_TYPE_DEBTPRESSINGAPPLY).equals(applyType)
				|| WorkflowConstant.APPLI_TYPE_DEBTPOOLPRESSINGAPPLY.equals(applyType)) {
			// 催收
			if (ScfBasConstant.FACT_TYPE_INLAND.equals(factType) || ScfBasConstant.FACT_TYPE_REVERSE.equals(factType)) {
				bean.setNoticeType(BNteConstant.NOTICE_TYPE_DEBTPRESSINGAPPLY);
				...
				bean.setNoticeType(BNteConstant.NOTICE_TYPE_110);
				noticeNo = bSysSerialNoService.genSerialNo(ScfBasConstant.BUSSNO_TYPE_YSZKC_C_NOTICE_NO);
			} else if (ScfBasConstant.FACT_TYPE_DOUBLE_FACT.equals(factType)) {
				bean.setNoticeType(BNteConstant.NOTICE_TYPE_111);
			} else if (ScfBasConstant.FACT_TYPE_CREDIT_FACT.equals(factType)) {
				bean.setNoticeType(BNteConstant.NOTICE_TYPE_112);
			} else if (ScfBasConstant.FACT_TYPE_LEASE_FACT.equals(factType)) {
				bean.setNoticeType(BNteConstant.NOTICE_TYPE_113);
			} else if (ScfBasConstant.FACT_TYPE_DEBT_PLEDGE.equals(factType)) {
				bean.setNoticeType(BNteConstant.NOTICE_TYPE_115);
			}

		}
		/*
		 * else if((WorkflowConstant.APPLI_TYPE_FINANINGEXTENSIONAPPLY).equals(
		 * applyType)){//融资展期申请
		 * bean.setNoticeType(BNteConstant.NOTICE_TYPE_FINANINGEXTENSIONAPPLY);
		 * } else
		 * if((WorkflowConstant.APPLI_TYPE_INVOICEFEESUPPLEMENTAPPLY).equals(
		 * applyType)){//发票费用补录申请
		 * bean.setNoticeType(BNteConstant.NOTICE_TYPE_INVOICEFEESUPPLEMENTAPPLY
		 * ); } else
		 * if((WorkflowConstant.APPLI_TYPE_AFTERCHARGETOCOLLECTAPPLY).equals(
		 * applyType)){//后收费用收取申请
		 * bean.setNoticeType(BNteConstant.NOTICE_TYPE_AFTERCHARGETOCOLLECTAPPLY
		 * ); }
		 */
		List<RBcpAppliBussInfo> list = rBcpAppliBussInfoService.queryAppliBussInfoBillsNo(appno);// 获取申请基本信息
		if (list == null || list.size() <= 0) {
	...
	public void modifyNoticeBaseInfo(String appno, String noticeBaseStatus) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (BNteConstant.NOTICE_BASE_STATUS_CONFIRM_APPROVED.equals(noticeBaseStatus)) {
	...
	public void addDebtCancelNoticeByAppno(String appno) throws ScubeBizException {
		...
		// 调用应收账款管理公共方法
		this.saveFactoringNotice(debtVO, amount, BNteConstant.NOTICE_TYPE_25);
	}
	...
	public void saveFactoringNotice(AppliBussInfoVO debtVO, BigDecimal amount, String noticeType) throws ScubeBizException {
		...
		// NoticeQryVO commonQueryVO = new NoticeQryVO();
		// commonQueryVO.setAppno(debtVO.getAppno());
		// 通过appno获取通知书
		// List tblNoticeBaseInfo =
		// this.getFactoringNoticeBaseInfoListByParam(commonQueryVO);
		BNteNoticeBase bNteNoticeBase = new BNteNoticeBase();
		...
		// TblContDebtExtInfo tblContDebtExtInfo =
		// DAOUtils.getTblContDebtExtInfoDAO().findByProperty("TblContDebtExtInfo",
		// "businessNo", mastContno);
		BCntDebtExtInfo tblContDebtExtInfo = bCntDebtExtInfoService.queryContDebtExInfoByMastContno(debtVO.getMastContno(), debtVO.getCustcdBuyer());
		String factType = "";
		String cont_noticeType = "";
		if (tblContDebtInfo != null) {
			factType = tblContDebtInfo.getFactType();
			cont_noticeType = tblContDebtExtInfo.getNoticeType();
			if (ScfBasConstant.FACT_TYPE_INLAND.equals(factType) || ScfBasConstant.FACT_TYPE_REVERSE.equals(factType)) {
				if (StringUtil.isStrNotEmpty(cont_noticeType) && RBcpDebtConstant.FACTORY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_100;
					// TODO
					// String noticeNo;
				} else if (RBcpDebtConstant.FACTORY_ONLY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_125;
					// TODO
					// String noticeNo;
				} else {
					noticeType = BNteConstant.NOTICE_TYPE_13;
					// TODO
					// String noticeNo;
				}
					...
					|| ScfBasConstant.FACT_TYPE_EXPORT_POOL_PLEDGE.equals(factType)) {
				if (StringUtil.isStrNotEmpty(cont_noticeType) && RBcpDebtConstant.FACTORY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_102;
					// TODO
					// String noticeNo;
				} else if (RBcpDebtConstant.FACTORY_ONLY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_125;
					...
					noticeType = BNteConstant.NOTICE_TYPE_101;
					// TODO
					// String noticeNo;
				}
			} else if (ScfBasConstant.FACT_TYPE_DOUBLE_FACT.equals(factType)) {
				if (StringUtil.isStrNotEmpty(cont_noticeType) && RBcpDebtConstant.FACTORY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_104;
					// TODO
					// String noticeNo;
				} else if (RBcpDebtConstant.FACTORY_ONLY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_125;
					...
					noticeType = BNteConstant.NOTICE_TYPE_103;
					// TODO
					// String noticeNo;
				}
			} else if (ScfBasConstant.FACT_TYPE_CREDIT_FACT.equals(factType)) {
				if (StringUtil.isStrNotEmpty(cont_noticeType) && RBcpDebtConstant.FACTORY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_106;
					// TODO
					// String noticeNo;
				} else if (RBcpDebtConstant.FACTORY_ONLY_NOTICE_TYPE.equals(cont_noticeType)) {
					noticeType = BNteConstant.NOTICE_TYPE_125;
					...
					noticeType = BNteConstant.NOTICE_TYPE_105;
					// TODO
					// String noticeNo;
				}
			} else if (ScfBasConstant.FACT_TYPE_LEASE_FACT.equals(factType)) {
				noticeType = BNteConstant.NOTICE_TYPE_107;
				// TODO
				// String noticeNo;
			} else if (ScfBasConstant.FACT_TYPE_DEBT_PLEDGE.equals(factType)) {
				noticeType = BNteConstant.NOTICE_TYPE_114;
				// TODO
				// String noticeNo;
			}
		...
		if (list != null && list.size() < 1) {// 新增
			...
			noticeBaseInfo.setBrcode(tblContDebtInfo.getBrcode());// 经办机构
			...
			noticeBaseInfo.setStatus(BNteConstant.NOTICE_BASE_STATUS_UNCONFIRM);
	...
	public String addNoticeBaseInfo(BPbcAppliBaseInfo bPbcAppliBaseInfo, BCrrGntyCon bCrrGntyCon, BCntMprotBaseInfo bCntMprotBaseInfo,
		...
		if (BNteConstant.NOTICE_TYPE_83.equals(noticeType)) {
			bNteNoticeBase.setStatus(BNteConstant.NOTICE_BASE_STATUS_CONFIRM_APPROVED);// 已签发
		}
		...
		// TODO
		/*
		 * if(BNteConstant.NOTICE_TYPE_1.equals(noticeType)){ List
		 ...
		 */
		if (BNteConstant.NOTICE_TYPE_22.equals(noticeType)) {
	...
	public void addAdvWorthCompensateNoticeInfo(BNteNoticeBase bNteNoticeBase) throws ScubeBizException {
		...
		bNteNoticeBase2.setStatus(BNteConstant.NOTICE_BASE_STATUS_UNCONFIRM);
...
//		ABcpAppliImprestBuss abcpAppliImprestBuss = aBcpAppliImprestBussService.findABcpAppliImprestBussByAppno(appno);
		...
		if(bPbcAppliBaseInfo.getSupplyChainPdId().equals(ScfBasConstant.PRODUCT_TYPE_SMALL_WH))//担保提货
			bNteNoticeBase.setNoticeType(BNteConstant.NOTICE_TYPE_80);// 担保提货通知书
		else if(bPbcAppliBaseInfo.getSupplyChainPdId().equals(ScfBasConstant.PRODUCT_TYPE_XPHH))//先票款后货
			bNteNoticeBase.setNoticeType(BNteConstant.NOTICE_TYPE_83);// 预付款提货通知书
	...
	public void saveFourMarginNoticBaseInfo(String appno, FourWareLoanVO vo) throws ScubeBizException {
		...
		if (noticebase == null || noticebase.size() < 1) {// 新增
			...
			tblNoticeBaseInfo.setNoticeType(BNteConstant.NOTICE_TYPE_85);// 预付款差额退款通知书
			tblNoticeBaseInfo.setStatus(BNteConstant.NOTICE_BASE_STATUS_UNCONFIRM);
...
//		SBcpAppliLowerPrice sBcpAppliLowerPrice = sBcpAppliLowerPriceService.findsaveAppliLowPriceByAppno(appno);
		SBcpAppliLowerPrice sBcpAppliLowerPrice = null;
		...
		List noticeBaseInfoList = getNoticeBaseInfoByAppno(appno);
		if (noticeBaseInfoList != null && noticeBaseInfoList.size() > 0) {
			for (int i = 0; i < noticeBaseInfoList.size(); i++) {
				BNteNoticeBase tblNoticeBaseInfo = (com.huateng.scf.bas.nte.model.BNteNoticeBase) noticeBaseInfoList.get(i);
				...
				if ((tblNoticeBaseInfo.getNoticeType()).equals(NoticeConstant.NOTICE_TYPE_1)) {// 监管下限
					...
					updateBNteNoticeBase(tblNoticeBaseInfo);
