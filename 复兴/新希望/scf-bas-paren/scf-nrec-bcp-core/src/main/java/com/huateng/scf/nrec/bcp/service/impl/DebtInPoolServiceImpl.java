 ...
 * lihao		2017年6月20日上午11:04:54	      新增
 *
 *            </pre>
 */
@ScubeService
@Service(IDebtInPoolService.BeanId)
public class DebtInPoolServiceImpl implements IDebtInPoolService, DealEndFlowService {
	...
	@Override
	@Transactional
	public String saveOrUpdateInPoolInfo(RBcpAppliBussInfo rBcpAppliBussInfo, List<RBcpAppliBussDtl> ds) {
		// 1.前台数据校验
		...
		if (StringUtils.isEmpty(rBcpAppliBussInfo.getAppno())) {
			String businessnoType = WorkflowConstant.APPLI_TYPE_ACCOUNTS_RECEIVABLE_INPOOL_APPLY;// 3266
			String workApplyType = "N-AccountsReceivableInPool";

			appno = bSysSerialNoService.genSerialNo(ScfBasConstant.BUSSNO_TYPE_YSRC_T_APP_NO);// 申请流水号
			...
			updatePoolInfo(rBcpAppliBussInfo, ds);
		}
		return appno;
	}
	...
	@Override
	@Transactional
	public void updatePoolInfo(RBcpAppliBussInfo rBcpAppliBussInfo, List<RBcpAppliBussDtl> ds) {
		// 1.申请基本信息
		...
		bPbcAppliBaseInfo1.setAmt(rBcpAppliBussInfo.getTotalDebtAmount());// 本次入池总金额
		try {
	...
	@Override
	@Transactional
	public void submitInPoolInfo(RBcpAppliBussInfo rBcpAppliBussInfo, List<RBcpAppliBussDtl> ds) {
	...
	@Override
	@Transactional
	public void submitInPoolInfoApply(String appno) {
		// 1.数据校验
	...
	// 申请入池单据校验规则
	@Override
	public void checkInpoolDebtRulses(List<RBcpDebtInfoVO> ds) {
		// 只是校验数据是否有效

		for (RBcpDebtInfoVO rBcpDebtInfoVO : ds) {
			// 1.单据状态必须为已转让
			if (!rBcpDebtInfoVO.getStatus().equals(DebtConstants.INVOICE_STATUS_PURCHASED)) {
				throw new ScubeBizException("凭证编号为" + rBcpDebtInfoVO.getBillsNo() + "单据信息单据状态不为已转让，不可申请入池！");
			}
			// 2.单据不可为已商纠
			if (rBcpDebtInfoVO.getIssueFlag() != null && rBcpDebtInfoVO.getIssueFlag().equals("1")) {
				throw new ScubeBizException("凭证编号为" + rBcpDebtInfoVO.getBillsNo() + "单据有商业纠纷，不可申请入池！");
			}
			// 3.单据的融资余额不可以为0
			...
			// loanRemainingAmount这个字段放过款则为0
			if (compareTo == 0) {// 只要两者不相等，说明这张借据放款过。并且没有全部还清。
				throw new ScubeBizException("凭证编号为" + rBcpDebtInfoVO.getBillsNo() + "单据已涉及放款业务，不可申请入池！");
			}
			// 4.单据不可为已入池
			if (rBcpDebtInfoVO.getPoolFlag() != null && rBcpDebtInfoVO.getPoolFlag().equals("1")) {
				throw new ScubeBizException("凭证编号为" + rBcpDebtInfoVO.getBillsNo() + "单据已入池，不可再申请入池！");
			}
			// 5.需要为未被锁定
			if (rBcpDebtInfoVO.getIsLocked().equals("1")) {
				throw new ScubeBizException("凭证编号为" + rBcpDebtInfoVO.getBillsNo() + "单据已被锁定，不可申请入池！");
			}
		}
		return;
	}
	...
	public void doWorkForPass(String appNo) {
		...
		String invokPhase = ScfBasConstant.INVOKE_PHASE_IN_POOL;// 转让nag申请阶段
		String callerpath = "DebtInPoolServiceImpl";
		...
		try {
			for (RBcpDebtInfoVO rBcpDebtInfo : rBcpDebtInfoVOList) {
				...
				rBcpDebtBussDtl.setMoney(rBcpDebtInfo.getRemainingAmount());// 入池的金额是单据的应收账款余额
		...
		// 4.池信息修改   updateRBcpDebtPoolDebtAmount
		//RBcpDebtPool debtPool, BCntBcpInfoVO baseInfo, String invokPhase
		...
		if(StringUtils.isEmpty(mastContno)){
			log.debug("入池时，合同号丢失！");
			throw new ScubeBizException("应收账款入池操作失败！");
		}
		...
		try {
			nRBcpDebtPoolService.updatePoolRemainAmount(debtPool,baseInfo,ScfBasConstant.INVOKE_PHASE_IN_POOL);
		} catch (Exception e) {
	...
	@Override
	public Page queryPoolBPbcAppliBaseInfoByPage(int pageNo, int pageSize, BPbcAppliBaseInfo bPbcAppliBaseInfo) throws ScubeBizException {
		if (bPbcAppliBaseInfo == null || StringUtils.isEmpty(bPbcAppliBaseInfo.getMastContno())) {
			return null;
		}
		if (StringUtils.isEmpty(bPbcAppliBaseInfo.getAppliType())) {
			bPbcAppliBaseInfo.setAppliType("3266,3267");
		}
		Page page = iBPbcAppliBaseInfoService.queryBPbcAppliBaseInfoByPage(pageNo, pageSize, bPbcAppliBaseInfo);
		return page;
	}
