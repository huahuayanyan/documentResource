 ...
 * huangshuidan		     2017年5月25日下午7:52:11           新增
 *
 * </pre>
 */
@ScubeService
@Service("prodPricingTableRuleAdapter")
public class ProdPricingTableRuleAdapter implements IScfAbsAdapter{
	...
	public List<? extends IScfAbsOutput> process(IScfAbsInput<? extends IScfAbsInput> params) throws ScubeBizException {
		...
		PricingGroup pricingGroupParm=new PricingGroup();
		if(null!=inputParam){
...
//		
//		log.info("--------------outProductInfo:"+outProductInfo + "");
//		log.info("--------------定价组ID:"+pricingGroupParm.getPricId());
...
//		scfCustBaseInfoOutput.setProductInfo(outProductInfo);
//		outputs.add(scfCustBaseInfoOutput);
		...
		inputParams.put(ScfBasConstant.RULES_SET_VERSION,
				ScfMessageUtil.transSysMsgByKey(ScfBasConstant.RULES_SET_VERSION_PRODPRICINGTABLE));// 规则集版本
