 ...
 * </pre>
 */
public final class WordNoticeUtil
{

	private static String characterEncoding = "UTF-8";
	private static String doccharacterEncoding = "GGB2312";
	private final static String DOWNLOAD_PATH = "AuthorizationFromParamsUrl";

	private WordNoticeUtil()
	{
	}
	 ...
	 * @throws Exception
	 */
	public static void reportMultiHtml(String path, Map paras, HttpServletRequest request, HttpServletResponse response)
			throws Exception
	{
       ...
       // exporter.setParameter(JRHtmlExporterParameter.IS_REMOVE_EMPTY_SPACE_BETWEEN_ROWS, Boolean.TRUE);
		...
		docExporter.setParameter(JRExporterParameter.JASPER_PRINT_LIST, jaspers);
		docExporter.setParameter(JRExporterParameter.CHARACTER_ENCODING, doccharacterEncoding);
		docExporter.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, path);
		docExporter.exportReport();
		
//		String appno = paras.get("appno").toString();
//		File file = new File(path);
//		String fileName = file.getName();
//		fileName = appno+fileName;
...
//		copyFile(path, newpath);
	...
	public void html(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		sb.append("<!DOCTYPE html>" + "<html lang='en'>" + "<head>" + "    <meta charset='UTF-8'>"
				+ "    <title>print</title>" + "    <script type='text/javascript'>" + "        function htPrint(){"
				...
				+ "                if(childTables){" + "                    for(var i=1;i<childTables.length;i++){"// 从第二个开始分页
				...
				+ "<iframe name='htPrintFrame' id='htPrintFrame' src='http://" + hostAddress + ":" + port
