 ...
 */
var pooldsptConListController = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
    $scope.conditionForm={
        financingType:"2",//融资模式是池融资
        conStatus:"01",//查有效合同
            ...
            promise.then(function(res){
                if (res.errorCode =="SUC"){
                    if(res.contextData.value != "0"){
                        ...
                        $state.go("pooldsptRegAdd");
                    }else{
                        $common.get_tipDialog("该合同不存在可登记纠纷的应收账款！",'showError');
                    }
                }else{
                    $common.get_tipDialog(res.errorMsg,'showError');
                }
