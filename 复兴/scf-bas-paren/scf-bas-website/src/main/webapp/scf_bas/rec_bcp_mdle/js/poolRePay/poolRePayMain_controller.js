 ...
 * Create at:   2016/9/29
 */
var poolRePayMainController =  function($scope,$common,$state,dataDeliver){
    var data = dataDeliver.getter();
    var appNo = data.appNo;
    $scope.poolRePay = {};
    $scope.submit=true;
    if($state.current.name=="bsystasktab.poolRePayWrite"){
        queryPoolRePayMainInfo();
        returnAccountSource();
        balanceAccountSource();
        $scope.submit=false;
    }
    if($state.current.name=="bsystasktab.poolRePayCheck"){
        queryPoolRePayMainInfo();
        returnAccountSource();
        balanceAccountSource();
    }
