 ...
 * Create at:   2016/4/21
 */
var CreditBindAddController =  function(dialogScope,parentScope,$common,flag,row){
    ...
    if(flag == "add"){
        ...
        dialogScope.self_hide=true;
    }
   //额度类型
    dialogScope.creditTypeContent = {
        type: "table",//展示类型table  tree
        url: "bIcrTypeService/findBIcrTypeByPage",//请求url
        column: [ "creditType"]//展示内容
    };
    dialogScope.$watch("otherform.creditType", function () {
        dialogScope.creditTypeContent.param = {
            creditType: dialogScope.otherform.creditType,
        };
    });
    dialogScope.confirm_onClick=function(){
        if($.doValidate("otherform")){//校验数据格式
            if(flag == "add"){
                var primise = $common.get_asyncData("bPrdCreditBindService/add", {bPrdCreditBind: dialogScope.otherform});
                ...
                var primise = $common.get_asyncData("bPrdCreditBindService/update", {bPrdCreditBind: dialogScope.otherform});
                primise.then(function (res) {
                    if (res.errorCode == "SUC") {
                        parentScope.queryTable();
                        parentScope.selected.select = [];
