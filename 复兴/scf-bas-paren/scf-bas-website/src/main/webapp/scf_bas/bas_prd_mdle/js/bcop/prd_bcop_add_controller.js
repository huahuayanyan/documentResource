 ...
 * Create at:   2016/4/21
 */
var PrdBcopAddController =  function(dialogScope,parentScope,$common,flag,row){
    dialogScope.otherform={//初始化账户       brNameContent,brNameExtent,cnameContent,cnameExtent
        bcopName:"",//页面上的客户名称
        bcopProg:"",
    };
    if(flag == "add"){
        dialogScope.hide=true;//新增的时候隐藏。
    ...
    dialogScope.confirm_onClick=function(){
        if($.doValidate("otherform")){//校验数据格式
            if(flag == "add"){//如果是新增，先去查重，如果有对应的组件，则提示，该组件已经被使用，不可新增！
                var bPrdBcop={//bcopName，bcopProg这两者都一样就是重复了。
                    bcopName:dialogScope.otherform.bcopName,
                    bcopProg:dialogScope.otherform.bcopProg,
                }
                var primise = $common.get_asyncData("bPrdBcopService/findBPrdBcopByBPrd",{bPrdBcop:bPrdBcop});
                primise.then(function(res){
                    if (res.errorCode == "SUC") {
                        ...
                        if (datal!=null&&datal.bcopName == dialogScope.otherform.bcopName &&datal.bcopProg == dialogScope.otherform.bcopProg) {//查到了数据
                            $common.get_tipDialog("该组件已经存在，不可新增！", 'showError');
                        } else {//
                            var primise = $common.get_asyncData("bPrdBcopService/add", {bPrdBcop: dialogScope.otherform});
                            primise.then(function (res) {
                                if (res.errorCode == "SUC") {
                ...
                primise.then(function(res){
                    if (res.errorCode == "SUC") {
                        ...
                        if (datal!=null&&datal.bcopName == dialogScope.otherform.bcopName && datal.bcopProg == dialogScope.otherform.bcopProg && datal.id !=dialogScope.otherform.id) {//查到了数据
                            $common.get_tipDialog("该组件信息已经存在，不可修改成此组件！", 'showError');
                        } else {//
                            var primise = $common.get_asyncData("bPrdBcopService/update", {bPrdBcop: dialogScope.otherform});
                            primise.then(function (res) {
                                if (res.errorCode == "SUC") {
                                    parentScope.queryTable();
                                    parentScope.selected.select=[];
