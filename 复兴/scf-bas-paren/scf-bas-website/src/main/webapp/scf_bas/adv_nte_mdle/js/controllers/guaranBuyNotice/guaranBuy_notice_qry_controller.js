...
//差额退款通知书列表
var guaranBuyNoticeQryController = function ($scope, $common, $state, dataDeliver) {
    ...
    function productSource(){
        //业务品种下拉框
        var dataItemsDO = {parentId:"2000",productId:"1210010"};
    ...
    $scope.queryTable = function (paramData) {
        ...
        var promise = $common.get_asyncData("guaranBuyNoticeService/queryGuaranBuyNteByPage", data);
        promise.then(function (res) {
            var data1;
            if (res.errorCode == "SUC") {
...
//查看操作！
    $scope.column = function(row){
        $common.get_ngDialog('guaranBuyNotice/guaranBuy_notice_qry_view.html', $scope
            ,["$scope",function(dialogScope){
                guaranBuyNoticeQryViewController(dialogScope,$scope,$common,row);
            }],"差额退款通知书基本信息",{showClose:true});
    };
    //重置按钮
    $scope.rebort_onClick = function () {
        $scope.$apply(function(){
            $scope.conditionForm = {productId:"1210010",noticeType:"71"};
