...
//差额退款通知书列表
var fcsDvyChkNoticeQryController = function ($scope, $common, $state, dataDeliver) {
...
//查看操作！
    $scope.column = function(row){
        $common.get_ngDialog('fcsDvyChkNotice/fcsDvyChk_notice_qry_view.html', $scope
            ,["$scope",function(dialogScope){
                fcsDvyChkNoticeQryViewController(dialogScope,$scope,$common,row);
            }],"发货对账通知书基本信息",{showClose:true});
    };
    //重置按钮
    $scope.rebort_onClick = function () {
        $scope.$apply(function(){
            $scope.conditionForm = {productId:"1210020",noticeType:"99"};
