...
var nrecBcpBackTransferListController = function($scope,$common,$state,dataDeliver) {
    $scope.conditionForm={//申请类型 3204，应收账款融资反转让，3284 应收账款反转让申请(池),3253 信保保理反转让,3254 双保理反转让,3255 租赁保理反转让
        appliType:"3204,3284,3253,3254,3255"
    };
    ...
    $scope.$on('interceptEvent',function(event,data) {
        ...
        $common.get_asyncData('nfactoringDebtBackService/submit',{appNo:$scope.selected.select[0].appno}).then(function(res){
            if (res.errorCode =="SUC"){
                $scope.queryTable();
                $scope.selected.select=[];
