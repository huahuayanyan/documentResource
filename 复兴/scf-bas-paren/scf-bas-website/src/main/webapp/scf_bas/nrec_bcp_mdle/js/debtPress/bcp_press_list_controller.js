...
var nrecBcpPressListController = function($scope,$common,$state,dataDeliver) {
    $scope.conditionForm={//申请类型 3224，应收账款融资催收，3225 应收账款池融资催收，3226 信保保理催收, 3227 双保理催收,3228 租赁保理催收
        appliType:"3224,3225,3226,3227,3228"
    };
    ...
    $scope.$on('interceptEvent',function(event,data) {
        ...
        $common.get_asyncData('ndebtPressApplyService/submit',{appNo:$scope.selected.select[0].appno}).then(function(res){
            if (res.errorCode =="SUC"){
                $scope.queryTable();
                $scope.selected.select=[];
