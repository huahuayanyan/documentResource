	...
	/*$scope.source={baseinfo:
	 ...
	 $scope.conf= {//初始化分页对象
	    ...
	    $scope.datasource={//初始化表格对象
	        ...
	        $scope.billnormalconf= {//初始化分页对象
	    	        pageNo: 1,
	    	        pageSize: 10,
	    	        totalRecord: 0};
	    	    $scope.billnormaldatasource={//初始化表格对象
	    	            ds: []
	    	        };
	    	        $scope.billnormalselected={//初始化对象
	    	            select: []
	    	        };
	     ...
	     /*   if(!$scope.taskform)
	        ...
	        function queryBillNormal(){
	            ...
	            var promise=$common.get_asyncData("adjustPriceBillService/getBillnormalQueryPage",data);
	            var datas;
	            promise.then(function (res) {
	                if(res.errorCode=="SUC"){
	                	datas = res.contextData;
	                    $scope.billnormaldatasource.ds = datas.records;
	                    $scope.billnormalconf.totalRecord = datas.totalRecord;
	                }else{
	                    $common.get_tipDialog(res.errorMsg,'showError');
	                }
	            })
	        ...
	        $scope.add_onClick = function(){
	            $common.get_ngDialog('adjustpricebill/adjustPriceApplicationAdjustPriceAdd.html', $scope
	        ...
	        $scope.update_onClick = function(){
	            if($scope.selected.select.length != 1){
	                ...
	                $common.get_ngDialog('adjustpricebill/adjustPriceApplicationAdjustPriceUpdate.html', $scope
	                    , ["$scope", function (dialogScope) {
	        ...
	        $scope.save_onClick = function()
	        {
	            ...
	            if($.doValidate("otherform")) {
	                ...
	                if(totPrice = 0){
	                	...
	                	$scope.otherform.productId='1202';
