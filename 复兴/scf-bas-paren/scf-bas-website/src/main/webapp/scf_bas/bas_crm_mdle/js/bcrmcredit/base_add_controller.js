...
var BaseAddController = function($scope,$common,$state,dataDeliver){
		    ...
		    $scope.queryTable = function(paramData){
		        ...
		        if(flag == "add"){
		        	var ctd="";
		        	if(data1.bCrmBaseInfo){
		        		ctd = data1.bCrmBaseInfo.custcd;
		        	}else{
		        		ctd = $scope.otherform.custcd;
		        	}
		        	data= {
				            pageNo:$scope.conf.pageNo,
				            pageSize:$scope.conf.pageSize,
		        ...
		        var promiseCon = $common.get_asyncData("bCrmContactsInfoService/findContractsInfoByPage",data);
		        promiseCon.then(function(res){
		            var data;
		            if (res.errorCode =="SUC"){
		                data = res.contextData;
		                $scope.datasourceCon.ds = data.records;
	        ...
	        $common.get_ngDialog('bcrmcredit/contractDialog.html', $scope,["$scope",function(dialogScope){
		            cntDialogController(dialogScope,$scope,$common,"cntadd");
		            }],"新增操作",{showClose:true});
		    };
		   //弹出联系人修改对话框
		    $scope.cntUpdate_onClick = function(){
		        if($scope.selected.select.length != 1){
		            ...
		            $common.get_ngDialog('bcrmcredit/contractDialog.html',$scope
		                ,["$scope",function(dialogScope){
		                	cntDialogController(dialogScope,$scope,$common,"cntupdate");
		                }],"修改操作",{showClose:true});
		        };
		    };
		    $scope.cntDelete_onClick = function(){
		        if($scope.selected.select.length <= 0){
		            ...
		            $common.get_tipDialog('确定删除吗！','showTip').then(function(){
		                var custcds= $.map($scope.selected.select,function(n){
		                    return n.id;
		                });
		               
		                var primise = $common.get_asyncData('bCrmContactsInfoService/delete',{key:custcds});
   ...
   //前台页面赋值(不通过flag去决定是否到后台查询数据，而是什么时候都要查询，防止，新增成功之后，去账户信息等页面，再跳转回此页面)
    ...
    primiseA.then(function(res){
        if (res.errorCode =="SUC") {
            ...
            //省市联动
            $scope.areaContent_two.param.upCode=res.contextData.province;
            $scope.areaContent_three.param.upCode=res.contextData.city;
    ...
    if (flag == "update"){//先把之前的客户经理缺失的给加上去
        $scope.otherform.creditFlag="1";//防止是一个已注册的非授信客户转成授信客户时候，授信客户标识不为1
        $scope.hide2=true;
    ...
    else if(flag == "add" && (!data1.bCrmBaseInfo)){
   ...
   $scope.industryTypeContent = {
    ...
    function checkFormRules(){
        ...
        if($scope.otherform.fax){
            ...
            if( 0 < fax.length < 30){
                var fax_regTel = new RegExp(/^1\d{10}$|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$/);
    ...
    $scope.confirm_onClick = function(){
        if($.doValidate("otherform")) {
        	var upflag=$scope.otherform.upFlag;
        	var downflag=$scope.otherform.downFlag;
        	
        	if(upflag == "0" && downflag == "0"){
        		 $common.get_tipDialog("上游客户或下游客户至少选择一种客户类型！",'showError');
        		 return;
        	}
        	if(upflag == undefined && downflag == undefined){
       		 $common.get_tipDialog("上游客户或下游客户至少选择一种客户类型！",'showError');
       		 return;
       	}
            ...
            if(establishDate > sysDate){
                ...
                if(errorFlag){
                    ...
                    if (flag == "add") {
                        var primise2 = $common.get_asyncData("bCrmBaseInfoService/findBCrmBaseInfoByKey", {bCrmBaseInfo: bCrmBaseInfo});
