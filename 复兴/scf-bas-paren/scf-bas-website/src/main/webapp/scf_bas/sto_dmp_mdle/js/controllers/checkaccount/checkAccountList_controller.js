...
var checkAccountListController = function($scope,$common,$state,dataDeliver,$eventCommunicationChannel) {
    $scope.conditionForm = {productId:"1220020"};//初始化查询条件对象
    $scope.productSource = {
    		 ds:[]
        }
    ...
    $scope.dooextent = function(ds){
        ...
        $scope.conditionForm.recordBrcode=ds.brNo;
    };
//查询业务品种
    $scope.productSource = function(){
        ...
        promise.then(function(res){
            if (res.errorCode =="SUC"){
                ...
                $scope.conditionForm.productId="1220020";
                $scope.queryTable();
            }else{
   ...
   $scope.queryTable = function(){
       ...
       var data= {
        ...
        var promise = $common.get_asyncData("bPbcCheckAccountService/queryChkStockListByPage",data);
        promise.then(function(res){
            var data;
            if (res.errorCode =="SUC"){
    ...
    $scope.add_onClick = function(){
        var data = { productId:"1220020"};
        dataDeliver.setter(data);//传值[可选]
        $state.go("checkAccountAdd");//跳转
    };


    //对账详情
    $scope.column = function(row){
            $state.go("checkAccountDtl");
            var data= {
                type:'detail',
                baseInfo:row
            } ;
