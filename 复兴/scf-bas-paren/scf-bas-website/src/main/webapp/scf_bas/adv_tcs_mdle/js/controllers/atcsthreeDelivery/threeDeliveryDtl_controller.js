...
var threeDeliveryDtlController = function($scope,$common,dataDeliver,$state,$compile,$eventCommunicationChannel) {
    $scope.otherform={};
    $scope.threeDeliveryform={};
  ...
  /*  if(bData.flag=="task"){ //待办任务隐藏申请页面按钮
    ...
    if(appno==null){
        ...
        queryBaseInfo();
        //本次申请提货总金额
        $scope.threeDeliveryform.amount=0.0;
    }else{ //流程中的处理
...
//根据合同编号加载合同基本信息
    ...
    function queryDebtInfo(){
        ...
        promise.then(function (res) {
            if(res.errorCode=="SUC"){
                data = res.contextData;
                $.extend($scope.threeDeliveryform,res.contextData); //合并
            }else{
                $common.get_tipDialog(res.errorMsg,'showError');
            }
        })
    ...
    function queryDeliveryInfo(){
        ...
        var promise=$common.get_asyncData("aTcsThreeDeliveryService/getThreeDeliveryInfo",data);
        promise.then(function (res) {
            if(res.errorCode=="SUC"){
                data = res.contextData;
                if($scope.source.addBailAmount == null||$scope.source.addBailAmount ==''){
                }else{
                    $scope.threeDeliveryform.addBailAmount = $scope.source.addBailAmount;
                }
                $.extend($scope.threeDeliveryform,res.contextData); //合并
                dataDeliver.setter($scope.threeDeliveryform);
            }else{
                $common.get_tipDialog(res.errorMsg,'showError');
            }
        })
    ...
    $scope.coreNmdooextent = function(ds){
        $scope.threeDeliveryform.coreCustcd=ds.custcd;
        $scope.threeDeliveryform.coreNm=ds.cname;
    };
    $scope.$watch("threeDeliveryform.coreNm",function(){
        //ht-input-extent  需要用监听的方式来传额外的参数
        $scope.coreNmContent.param ={
            coreFlag:"1", //核心厂商
            cname:$scope.threeDeliveryform.cname
        };
    });
    ...
    $scope.dooextent = function(ds){
        $scope.threeDeliveryform.custcd=ds.custcd;
        $scope.threeDeliveryform.cname=ds.cname;
    };
    $scope.$watch("threeDeliveryform.cname",function(){
        //ht-input-extent  需要用监听的方式来传额外的参数
    ...
    $scope.productNamedooextent = function(ds){
        $scope.threeDeliveryform.productName=ds.productName;
        $scope.threeDeliveryform.productId=ds.productId; //业务品种
        $scope.threeDeliveryform.factType=ds.factType;
        $scope.threeDeliveryform.financingType=ds.financingType;
    };
    $scope.$watch("threeDeliveryform.productName",function(newVal,oldVal){
        //ht-input-extent  需要用监听的方式来传额外的参数
        $scope.productNameContent.param ={
            productName:$scope.threeDeliveryform.productName,
    ...
    $scope.add_onClick = function(){
        $common.get_ngDialog('atcsthreeDelivery/threeDeliveryMortListAdd.html', $scope
                ...
                dialogScope.queryTable = function(){
                    ...
                    promise.then(function (res) {
                        if(res.errorCode=="SUC"){
                          ...
                          /*  //mortgageNo 货物编号
                            for(var x in dialogScope.datasourceDialog.ds){//弹出框数据
                                for(var y in $scope.datasource.ds){//页面数据
                                    if(dialogScope.datasourceDialog.ds[x].mortgageNo == $scope.datasource.ds[y].mortgageNo){
                                        dialogScope.datasourceDialog.ds.splice(x,1);
                                    }
                                }
                            }*/
    ...
    $scope.update_onClick = function(){
        if($scope.selected.select.length != 1){
            ...
            $common.get_ngDialog('atcsthreeDelivery/threeDeliveryMortListDtl.html', $scope
                , ["$scope", function (dialogScope) {
                    ...
                    dialogScope.confirm_onClick = function () {
                        if($.doValidate("dialogform")) {
                            ...
                            $scope.threeDeliveryform.amount = totalamt;
                            //本次申请提货总金额的计算end
                            dialogScope.closeThisDialog();
    ...
    $scope.addBailAmount_onBlur = function(newVal,oldVal){
        ...
        //金额长度的校验
        //追加保证金长度校验
        var addBailAmount = $scope.threeDeliveryform.addBailAmount;
        if( checkLenght(addBailAmount.toString())>16)
        {
            $common.get_tipDialog('【本次追加保证金总额】系统允许最大输入长度为16位，请重新输入','showError');
            $scope.threeDeliveryform.addBailAmount="";;
            return false;
        }
        $scope.threeDeliveryform.loanWay=$scope.threeDeliveryform.lnciType;
            var data = {
                threeDeliveryVO: $scope.threeDeliveryform
            };
            var primise = $common.get_asyncData("aTcsThreeDeliveryService/getCanDeliveryAmt", data);
            primise.then(function (res) {
                if (res.errorCode == "SUC") {
                    data = res.contextData;
                    $scope.threeDeliveryform.canDeliveryAmt=data;
                } else {
                    $common.get_tipDialog(res.errorMsg, 'showError');
                }
            });
    ...
    $scope.save_onClick = function(){
        if($.doValidate("otherform")) {
           ...
           //追加保证金长度校验
           var addBailAmount = $scope.threeDeliveryform.addBailAmount;
            var openAmount = $scope.threeDeliveryform.openAmount;
            if( checkLenght(addBailAmount.toString())>16)
            {
                $common.get_tipDialog('【本次追加保证金总额】系统允许最大输入长度为16位，请重新输入','showError');
                return false;
           ...
           var msg = checkData();
            ...
            var primise = $common.get_asyncData("aTcsThreeDeliveryService/saveThreeDeliveryAppInfo", data);
            primise.then(function (res) {
                if (res.errorCode == "SUC") {
                    data = res.contextData;
                    $scope.threeDeliveryform.appno=data.value;
                    $scope.source.addBailAmount = $scope.threeDeliveryform.addBailAmount;//本次追加保证金总额--放到公共里面\
    ...
    $scope.sumbit_onClick = function(){
        if($.doValidate("otherform")) {
            ...
            var primise = $common.get_asyncData("aTcsThreeDeliveryService/saveThreeDeliveryAppInfoSubmit", data);
            primise.then(function (res) {
                if (res.errorCode == "SUC") {
                    //$scope.queryTable();
                    //$scope.selected.select = [];
    ...
    function checkData(){
        ...
        var addBailAmount = 0;
        var openAmount = 0;
        addBailAmount = $scope.threeDeliveryform.addBailAmount;
        openAmount  = $scope.threeDeliveryform.openAmount;
        ...
        var amount = $scope.threeDeliveryform.amount;
        var canDeliveryAmt = $scope.threeDeliveryform.canDeliveryAmt;
        if(!amount){
            amount=0;
        };
        if(!canDeliveryAmt){
        ...
        //	threeDeliveryAppInfoMortList_dataset.setValue("totprice",0);
        //}
