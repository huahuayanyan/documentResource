 ...
 * <p>@Description: </p>
 * <p>@Created time: Jun 16, 2005 4:29:07 PM</p>
...
public class Holiday implements Serializable{
	...
	public Holiday(String _strDate) throws Exception{
		...
		this.end_day_of_month =
			ca.getActualMaximum(java.util.Calendar.DAY_OF_MONTH);
		this.month_of_year =
			ca.get(java.util.Calendar.MONTH) - java.util.Calendar.JANUARY + 1;
		/*
		 * 星期几
		 */
	...
	public String toString() {
		...
		sb
			...
			.append("]Day Of Month[")
			.append(day_of_month)
			.append("] End Day Of Month[")
			.append(end_day_of_month)
			.append("]");
		if (this.isWeekEnd())
			sb.append(" [Week End]");
	...
	public boolean checkRun(String runtime) {
		...
		if (runtime.equals(BatchConstant.SUB_STEP_RUN_TIME_DAILY)) {
			// 每日运行
			return true;
		} else if (
			runtime.equals(BatchConstant.SUB_STEP_RUN_TIME_EVERY_TEN_DAYS)
				&& this.isaPeriodOfTenDays()) { // 每旬运行
			return true;
		} else if (
			runtime.equals(BatchConstant.SUB_STEP_RUN_TIME_MONTHLY)
				&& this.isMonthEnd()) { // 每月运行
			return true;
		} else if (
			runtime.equals(BatchConstant.SUB_STEP_RUN_TIME_EVERY_SEASON)
				&& this.isSeasonEnd()) { // 每季运行
			return true;
		} else if (
			runtime.equals(BatchConstant.SUB_STEP_RUN_TIME_EVERY_HALF_YEAR)
				&& this.isHalfYearEnd()) { // 每半年运行
			return true;
		} else if (
			runtime.equals(BatchConstant.SUB_STEP_RUN_TIME_YEARLY)
				&& this.isYearEnd()) { // 每年运行
			return true;
		}
		/*
		 ...
		 */
			...
			runtime.equals(BatchConstant.WEEKDAY_MONDAY)
				&& this.day_of_week == 1) { // 周一运行
			return true;
		} else if (
			runtime.equals(BatchConstant.WEEKDAY_TUESDAY)
				&& this.day_of_week == 2) { // 周二运行
			return true;
		} else if (
			runtime.equals(BatchConstant.WEEKDAY_WEDNESDAY)
				&& this.day_of_week == 3) { // 周三运行
			return true;
		} else if (
			runtime.equals(BatchConstant.WEEKDAY_THURSDAY)
				&& this.day_of_week == 4) { // 周四运行
			return true;
		} else if (
			runtime.equals(BatchConstant.WEEKDAY_FRIDAY)
				&& this.day_of_week == 5) { // 周五运行
			return true;
		} else if (
			runtime.equals(BatchConstant.WEEKDAY_SATURDAY)
				&& this.day_of_week == 6) { // 周六运行
			return true;
		} else if (
			runtime.equals(BatchConstant.WEEKDAY_SUNDAY)
				&& this.day_of_week == 7) { // 周日运行
			return true;
		}
		/*
		 * 指定为每月中的某日运行
	...
	/**added by wangpeng 20090924 BMS-2006 begin**/
	...
	/**added by wangpeng 20090924 BMS-2006 end**/
}
